...
# 1.4 Korrigierte Team Composition/Motivation Integration
...
# Debug: wie viele Zeilen hat das Team-Table?
message("Rows in old_final_corrected: ", nrow(old_final_corrected))
...
# Join mit normalisierten Schlüsseln
old_rounds_final <- old_rounds_with_rounds %>%
  left_join(old_final_corrected %>% select(-comm), 
            by = c("participant.code", "team_id", "task"))

# Check: Nicht gematchte Fälle
not_matched <- old_rounds_final %>%
  filter(is.na(team_composition_value) | is.na(team_motivation_value)) %>%
  distinct(participant.code, team_id, task)

print(paste("Nicht gematchte Fälle:", nrow(not_matched)))
print(head(not_matched))

old_rounds_final <- old_rounds_with_rounds %>%
  # join ohne comm, damit unterschiedliche Label-Formate nicht zu Missmatch führen
  left_join(old_final_corrected %>% select(-comm),
            by = c("participant.code", "team_id", "task")) %>%
  # Falls nach dem Join noch NAs auftauchen: hier kann man später Fallbacks ergänzen
  mutate(
    # hier coalesce nur symbolisch — falls du später einen zweiten Datenquelle hast, 
    # kannst du coalesce(team_composition_value, other_source) nutzen
    team_composition_value = team_composition_value,
    team_motivation_value = team_motivation_value
  ) %>%
...
message("Rows in old_rounds_final: ", nrow(old_rounds_final))

# -------------------------
# Debugging: welche Runden/Teilnehmer wurden nicht gematcht?
# -------------------------
not_matched <- old_rounds_with_rounds %>%
  anti_join(old_final_corrected %>% select(-comm), 
            by = c("participant.code", "team_id", "task")) %>%
  distinct(participant.code, team_id, task)

message("Anzahl nicht gematchter (participant.code, team_id, task): ", nrow(not_matched))
if(nrow(not_matched) > 0) print(head(not_matched, 20))
...
# ================================================================================
# TEIL 2: NEUE DATEN KORREKT AUFBEREITEN  
# ================================================================================

print("--- KORRIGIERE NEUE DATEN ---")
...
new_data_processing <- flow_clean %>%
  filter(task == "Math", comm %in% c("Chat", "Jitsi")) %>%
  group_by(participant.code, task, comm) %>%
  group_split() %>%
  map_dfr(~{
    group_data <- .x
...
new_rounds_final <- new_data_processing %>%
  # Füge Order hinzu
  left_join(order_summary_new, by = c("participant.code", "task", "comm")) %>%
  ...
  dplyr::select(participant.code, team_id, comm, task, difficulty, round, order,
         flow_score, stress_value, individual_motivation_value, 
         valence_value, arousal_value, information_sharing_value, 
         synchronization_value, team_composition_value, team_motivation_value)

print(paste("Neue Daten korrekt aufbereitet:", nrow(new_rounds_final), "Beobachtungen"))

# ================================================================================
# TEIL 3: FINALE INTEGRATION
# ================================================================================

print("--- FINALE INTEGRATION ---")
