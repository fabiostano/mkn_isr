...
data_old_performance <- read.csv("00-taskFeatures-All.csv")
```
...
```{r}
# Math Performance Analysis für Masterarbeit
# Analysiert Team-Performance basierend auf Action Logs
...
# data <- read.csv("your_data_file.csv")  # oder read.xlsx() etc.

# team_id ist bereits vorhanden (Format: session_team, z.B. eft5z7p2_P82P6RXPWR)
# Keine Umbennenung nötig

# 2. Einen Teilnehmer pro Team auswählen (da Action Logs team-weise identisch)
team_data <- data %>%
  group_by(team_id) %>%
  slice(1) %>%  # Nimmt den ersten Teilnehmer pro Team
  ungroup()

# 3. Math Order und Schwierigkeitsgrad-Mapping - FIXED
math_data <- team_data %>%
  select(participant.code, team_id, math_order,
         mathJitsi.3.player.action_log, mathJitsi.4.player.action_log, 
         mathJitsi.5.player.action_log, mathJitsi.6.player.action_log,
         mathChat.3.player.action_log, mathChat.4.player.action_log, 
         mathChat.5.player.action_log, mathChat.6.player.action_log) %>%
  # Identifiziere Condition basierend auf nicht-leeren Logs
  mutate(
    has_jitsi = !is.na(mathJitsi.3.player.action_log) & mathJitsi.3.player.action_log != "",
    has_chat = !is.na(mathChat.3.player.action_log) & mathChat.3.player.action_log != "",
    condition = case_when(
      has_jitsi ~ "jitsi",
      has_chat ~ "chat", 
      TRUE ~ "unknown"
  ...
  # Robuste Extraktion der Schwierigkeitsgrade
  rowwise() %>%
  mutate(
    # Extrahiere alle 4 Elemente direkt aus dem Vektor
    difficulty_round3 = if(length(math_order) >= 1) math_order[1] else NA_character_,
    difficulty_round4 = if(length(math_order) >= 2) math_order[2] else NA_character_,
    difficulty_round5 = if(length(math_order) >= 3) math_order[3] else NA_character_,
    difficulty_round6 = if(length(math_order) >= 4) math_order[4] else NA_character_
  ) %>%
  ungroup() %>%
  # Mappe Buchstaben zu lesbaren Namen
  mutate(
    difficulty_round3 = case_when(
      difficulty_round3 == "B" ~ "Easy",
      difficulty_round3 == "A" ~ "Optimal_Selected", 
      difficulty_round3 == "F" ~ "Optimal_Calibrated",
      difficulty_round3 == "O" ~ "Hard",
      is.na(difficulty_round3) ~ "Missing",
      TRUE ~ paste("Unknown:", difficulty_round3)
    ),
    difficulty_round4 = case_when(
      difficulty_round4 == "B" ~ "Easy",
      difficulty_round4 == "A" ~ "Optimal_Selected",
      difficulty_round4 == "F" ~ "Optimal_Calibrated", 
      difficulty_round4 == "O" ~ "Hard",
      is.na(difficulty_round4) ~ "Missing",
      TRUE ~ paste("Unknown:", difficulty_round4)
    ),
    difficulty_round5 = case_when(
      difficulty_round5 == "B" ~ "Easy",
      difficulty_round5 == "A" ~ "Optimal_Selected",
      difficulty_round5 == "F" ~ "Optimal_Calibrated",
      difficulty_round5 == "O" ~ "Hard", 
      is.na(difficulty_round5) ~ "Missing",
      TRUE ~ paste("Unknown:", difficulty_round5)
    ),
    difficulty_round6 = case_when(
      difficulty_round6 == "B" ~ "Easy",
      difficulty_round6 == "A" ~ "Optimal_Selected",
      difficulty_round6 == "F" ~ "Optimal_Calibrated",
      difficulty_round6 == "O" ~ "Hard",
      is.na(difficulty_round6) ~ "Missing",
      TRUE ~ paste("Unknown:", difficulty_round6)
    )
  )

# DEBUG: Schaue dir math_order an
cat("=== MATH ORDER DEBUG ===\n")
cat("Typ von math_order:", class(math_data$math_order[1]), "\n")
cat("Länge von math_order[1]:", length(math_data$math_order[1]), "\n")
cat("Erste 3 math_order Werte:\n")
for (i in 1:min(3, nrow(math_data))) {
  cat("Team", i, "- math_order:", paste(math_data$math_order[i], collapse=", "), "\n")
  cat("  Länge:", length(math_data$math_order[i]), "\n")
}

# Check der Schwierigkeitsgrad-Extraktion
cat("\n=== DIFFICULTY MAPPING CHECK ===\n")
sample_mapping <- math_data %>% 
  select(team_id, difficulty_round3, difficulty_round4, difficulty_round5, difficulty_round6) %>%
  head(5)
print(sample_mapping)

# Korrigierte Debug-Funktion - kompatibel mit älteren R-Versionen
debug_action_log <- function(action_log, log_name = "unknown") {
  cat("=== DEBUG", log_name, "===\n")
  if (is.na(action_log) || is.null(action_log)) {
    cat("Log ist NA oder NULL\n")
    return()
  }
  if (action_log == "") {
    cat("Log ist leer\n") 
  ...
  cat("Log Länge:", nchar(action_log), "\n")
  cat("Erste 300 Zeichen:", substr(action_log, 1, 300), "\n")
  
  # Parse Events (Semikolon-getrennt)
  clean_log <- str_replace(action_log, "^;", "")
  event_parts <- str_split(clean_log, ";\\[")[[1]]
  events <- sapply(event_parts, function(x) {
    if (!str_detect(x, "^\\[")) {  # str_detect statt str_starts_with
      x <- paste0("[", x)
    }
    x <- str_replace(x, "\\]$", "")
    x <- str_replace(x, "^\\[", "")
  ...
  cat("Gefundene Events:", length(events), "\n")
  
  # Zeige Trial-Paare (trialStart + submit/timeUp)
  trial_count <- 0
  for (i in seq_along(events)) {
    if (str_detect(events[i], "trialStart")) {
      trial_count <- trial_count + 1
      cat(paste0("\n--- TRIAL ", trial_count, " ---\n"))
      cat("Aufgabenstellung:", substr(events[i], 1, 100), "...\n")
      
      # Suche das zugehörige submit/timeUp
      if (i + 1 <= length(events) && str_detect(events[i + 1], "submit|timeUp")) {
        cat("Antworten:", substr(events[i + 1], 1, 100), "...\n")
      } else {
        cat("FEHLER: Kein submit/timeUp nach trialStart gefunden!\n")
      }
    }
  }
  
  cat(paste0("\nGesamt gefundene Trials: ", trial_count, "\n"))
  cat("========================\n")
}

# Verbesserte Parser-Funktion für Action Logs
parse_team_performance <- function(action_log) {
  if (is.na(action_log) || action_log == "" || is.null(action_log)) {
    return(list(
      total_trials = 0,
      successful_trials = 0,
      success_rate = NA,
  ...
  tryCatch({
    # Extrahiere alle Events - verbessertes Pattern
    events <- str_extract_all(action_log, "\\[([^\\[\\]]+)\\]")[[1]]
    ...
    trials <- list()
    current_trial_start <- NULL
    
    for (i in seq_along(events)) {
      event_str <- events[i]
      
      # Bereinige Event String
      event_clean <- str_replace_all(event_str, "^\\[|\\]$", "")
      
      # Splitte vorsichtiger - JSON kann Kommas in Strings haben
      if (str_detect(event_clean, "trialStart")) {
        # trialStart Event
        parts <- str_split(event_clean, ",", n = 3)[[1]]
        if (length(parts) >= 2) {
          timestamp <- suppressWarnings(as.numeric(str_trim(parts[1])))
          if (!is.na(timestamp)) {
            current_trial_start <- timestamp
          }
        }
      } else if (str_detect(event_clean, "submit|timeUp") && !is.null(current_trial_start)) {
        # submit/timeUp Event
        parts <- str_split(event_clean, ",")[[1]]
        
        if (length(parts) >= 14) {  # Mindestens timestamp + eventType + 12 values
          timestamp <- suppressWarnings(as.numeric(str_trim(parts[1])))
          event_type <- str_trim(parts[2])
          
          # Extrahiere die numerischen Werte (ab Position 3)
          numeric_parts <- parts[3:length(parts)]
          values <- suppressWarnings(as.numeric(str_trim(numeric_parts)))
          
          # Entferne NAs und prüfe ob genug Werte da sind
          valid_values <- values[!is.na(values)]
          
          if (length(valid_values) >= 12 && !is.na(timestamp)) {
            correct_sum1 <- valid_values[8]   # correctSum1
            correct_sum2 <- valid_values[10]  # correctSum2  
            correct_sum3 <- valid_values[12]  # correctSum3
            
            # Prüfe ob die Werte valide sind (0 oder 1)
            if (all(c(correct_sum1, correct_sum2, correct_sum3) %in% c(0, 1))) {
              team_success <- (correct_sum1 + correct_sum2 + correct_sum3) >= 2
              response_time <- timestamp - current_trial_start
              
              trials[[length(trials) + 1]] <- list(
                success = team_success,
                response_time = response_time,
                event_type = event_type,
                correct_sums = c(correct_sum1, correct_sum2, correct_sum3)
              )
            }
          }
        }
    ...
    successful_trials <- sum(sapply(trials, function(t) t$success))
    success_rate <- if (total_trials > 0) successful_trials / total_trials else NA
    response_times <- sapply(trials, function(t) t$response_time)
    avg_response_time <- mean(response_times, na.rm = TRUE)
    timeup_count <- sum(sapply(trials, function(t) t$event_type == "timeUp"))
...
# ARBEITS-VERSION: Kopiere die funktionierende Logik aus test_parse_detailed
parse_team_performance <- function(action_log) {
  if (is.na(action_log) || action_log == "" || is.null(action_log)) {
    return(list(
      total_trials = 0,
      ...
      avg_response_time = NA,
      timeup_count = 0,
      correct_trials = 0  # NEU: Anzahl korrekter Trials
    ))
  }
  
  tryCatch({
    # EXAKT die gleiche Logik wie in test_parse_detailed!
    clean_log <- str_replace(action_log, "^;", "")
    event_parts <- str_split(clean_log, ";\\[")[[1]]
    events <- sapply(event_parts, function(x) {
      if (!str_detect(x, "^\\[")) {
    ...
    for (i in seq_along(events)) {
      event_clean <- events[i]
      
      if (str_detect(event_clean, "trialStart")) {
        timestamp_match <- str_extract(event_clean, "^\\d+")
        if (!is.na(timestamp_match)) {
          current_trial_start <- as.numeric(timestamp_match)
        }
        ...
        if (length(parts) >= 14) {
          timestamp <- suppressWarnings(as.numeric(str_trim(parts[1])))
          event_type <- str_trim(parts[2])
          values <- suppressWarnings(as.numeric(str_trim(parts[3:14])))
          
          if (!is.na(timestamp) && length(values) == 12) {
            correct_sum1 <- values[8]   # correctSum1
            correct_sum2 <- values[10]  # correctSum2  
            correct_sum3 <- values[12]  # correctSum3
            
            # Behandle NAs als 0
            correct_sum1 <- ifelse(is.na(correct_sum1), 0, correct_sum1)
            correct_sum2 <- ifelse(is.na(correct_sum2), 0, correct_sum2)
            correct_sum3 <- ifelse(is.na(correct_sum3), 0, correct_sum3)
    ...
    total_trials <- length(trials)
    successful_trials <- sum(sapply(trials, function(t) t$success))
    correct_trials <- successful_trials  # NEU: Bei Math ist correct = successful
    success_rate <- if (total_trials > 0) successful_trials / total_trials else NA
    response_times <- sapply(trials, function(t) t$response_time)
    avg_response_time <- mean(response_times, na.rm = TRUE)
    timeup_count <- sum(sapply(trials, function(t) t$event_type == "timeUp"))
    ...
    return(list(
      ...
      avg_response_time = avg_response_time,
      timeup_count = timeup_count,
      correct_trials = correct_trials  # NEU
    ))
...
# SOFORT TESTEN:
cat("=== TESTE KORRIGIERTE PARSER-FUNKTION ===\n")
test_result_fixed <- parse_team_performance(team_data$mathJitsi.3.player.action_log[1])
print(test_result_fixed)

# 5. Performance für alle Math-Runden berechnen - CONDITION-AWARE
math_performance <- math_data %>%
  rowwise() %>%
  mutate(
    # Nur die relevante Condition berechnen
    round3_performance = case_when(
      condition == "jitsi" ~ list(parse_team_performance(mathJitsi.3.player.action_log)),
      condition == "chat" ~ list(parse_team_performance(mathChat.3.player.action_log)),
      TRUE ~ list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0))
    ),
    round4_performance = case_when(
      condition == "jitsi" ~ list(parse_team_performance(mathJitsi.4.player.action_log)),
      condition == "chat" ~ list(parse_team_performance(mathChat.4.player.action_log)),
      TRUE ~ list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0))
    ),
    round5_performance = case_when(
      condition == "jitsi" ~ list(parse_team_performance(mathJitsi.5.player.action_log)),
      condition == "chat" ~ list(parse_team_performance(mathChat.5.player.action_log)),
      TRUE ~ list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0))
    ),
    round6_performance = case_when(
      condition == "jitsi" ~ list(parse_team_performance(mathJitsi.6.player.action_log)),
      condition == "chat" ~ list(parse_team_performance(mathChat.6.player.action_log)),
      TRUE ~ list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0))
    )
  ) %>%
  ungroup()

# 6. Performance-Metriken nach SCHWIERIGKEITSGRAD gruppieren
reorganize_by_difficulty <- function(df) {
  # Erstelle leere Listen für jede Schwierigkeitsstufe
  difficulty_data <- df %>%
    mutate(
      easy_performance = list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0)),
      optimal_selected_performance = list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0)),
      optimal_calibrated_performance = list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0)),
      hard_performance = list(list(total_trials = 0, successful_trials = 0, success_rate = NA, avg_response_time = NA, timeup_count = 0))
    )
  
  # Ordne jede Runde der entsprechenden Schwierigkeitsstufe zu
  for (i in 1:nrow(difficulty_data)) {
    # Runde 3
    if (difficulty_data$difficulty_round3[i] == "Easy") {
      difficulty_data$easy_performance[[i]] <- difficulty_data$round3_performance[[i]]
    } else if (difficulty_data$difficulty_round3[i] == "Optimal_Selected") {
      difficulty_data$optimal_selected_performance[[i]] <- difficulty_data$round3_performance[[i]]
    } else if (difficulty_data$difficulty_round3[i] == "Optimal_Calibrated") {
      difficulty_data$optimal_calibrated_performance[[i]] <- difficulty_data$round3_performance[[i]]
    } else if (difficulty_data$difficulty_round3[i] == "Hard") {
      difficulty_data$hard_performance[[i]] <- difficulty_data$round3_performance[[i]]
    }
    
    # Runde 4
    if (difficulty_data$difficulty_round4[i] == "Easy") {
      difficulty_data$easy_performance[[i]] <- difficulty_data$round4_performance[[i]]
    } else if (difficulty_data$difficulty_round4[i] == "Optimal_Selected") {
      difficulty_data$optimal_selected_performance[[i]] <- difficulty_data$round4_performance[[i]]
    } else if (difficulty_data$difficulty_round4[i] == "Optimal_Calibrated") {
      difficulty_data$optimal_calibrated_performance[[i]] <- difficulty_data$round4_performance[[i]]
    } else if (difficulty_data$difficulty_round4[i] == "Hard") {
      difficulty_data$hard_performance[[i]] <- difficulty_data$round4_performance[[i]]
    }
    
    # Runde 5
    if (difficulty_data$difficulty_round5[i] == "Easy") {
      difficulty_data$easy_performance[[i]] <- difficulty_data$round5_performance[[i]]
    } else if (difficulty_data$difficulty_round5[i] == "Optimal_Selected") {
      difficulty_data$optimal_selected_performance[[i]] <- difficulty_data$round5_performance[[i]]
    } else if (difficulty_data$difficulty_round5[i] == "Optimal_Calibrated") {
      difficulty_data$optimal_calibrated_performance[[i]] <- difficulty_data$round5_performance[[i]]
    } else if (difficulty_data$difficulty_round5[i] == "Hard") {
      difficulty_data$hard_performance[[i]] <- difficulty_data$round5_performance[[i]]
    }
    
    # Runde 6
    if (difficulty_data$difficulty_round6[i] == "Easy") {
      difficulty_data$easy_performance[[i]] <- difficulty_data$round6_performance[[i]]
    } else if (difficulty_data$difficulty_round6[i] == "Optimal_Selected") {
      difficulty_data$optimal_selected_performance[[i]] <- difficulty_data$round6_performance[[i]]
    } else if (difficulty_data$difficulty_round6[i] == "Optimal_Calibrated") {
      difficulty_data$optimal_calibrated_performance[[i]] <- difficulty_data$round6_performance[[i]]
    } else if (difficulty_data$difficulty_round6[i] == "Hard") {
      difficulty_data$hard_performance[[i]] <- difficulty_data$round6_performance[[i]]
    }
  }
...
# Anwenden der Neuorganisation
math_performance_by_difficulty <- reorganize_by_difficulty(math_performance)

# 7. Extrahiere Performance-Metriken nach Schwierigkeitsgrad - ERWEITERT
extract_difficulty_metrics <- function(df) {
  df %>%
    mutate(
      # Easy Performance
      easy_success_rate = sapply(easy_performance, function(x) x$success_rate),
      easy_trials = sapply(easy_performance, function(x) x$total_trials),
      easy_timeups = sapply(easy_performance, function(x) x$timeup_count),
      easy_avg_time = sapply(easy_performance, function(x) x$avg_response_time),
      easy_correct_trials = sapply(easy_performance, function(x) x$correct_trials),  # NEU
      
      # Optimal Selected Performance
      optimal_selected_success_rate = sapply(optimal_selected_performance, function(x) x$success_rate),
      optimal_selected_trials = sapply(optimal_selected_performance, function(x) x$total_trials),
      optimal_selected_timeups = sapply(optimal_selected_performance, function(x) x$timeup_count),
      optimal_selected_avg_time = sapply(optimal_selected_performance, function(x) x$avg_response_time),
      optimal_selected_correct_trials = sapply(optimal_selected_performance, function(x) x$correct_trials),  # NEU
      
      # Optimal Calibrated Performance  
      optimal_calibrated_success_rate = sapply(optimal_calibrated_performance, function(x) x$success_rate),
      optimal_calibrated_trials = sapply(optimal_calibrated_performance, function(x) x$total_trials),
      optimal_calibrated_timeups = sapply(optimal_calibrated_performance, function(x) x$timeup_count),
      optimal_calibrated_avg_time = sapply(optimal_calibrated_performance, function(x) x$avg_response_time),
      optimal_calibrated_correct_trials = sapply(optimal_calibrated_performance, function(x) x$correct_trials),  # NEU
      
      # Hard Performance
      hard_success_rate = sapply(hard_performance, function(x) x$success_rate),
      hard_trials = sapply(hard_performance, function(x) x$total_trials),
      hard_timeups = sapply(hard_performance, function(x) x$timeup_count),
      hard_avg_time = sapply(hard_performance, function(x) x$avg_response_time),
      hard_correct_trials = sapply(hard_performance, function(x) x$correct_trials)  # NEU
    ) %>%
    # Entferne die list-columns 
    select(-c(easy_performance, optimal_selected_performance, optimal_calibrated_performance, hard_performance,
              round3_performance, round4_performance, round5_performance, round6_performance))
}

# Anwenden der Metrik-Extraktion
final_math_performance <- extract_difficulty_metrics(math_performance_by_difficulty)

# 7. Aggregierte Metriken berechnen - CONDITION-AWARE
final_math_performance <- final_math_performance %>%
  mutate(
    # Durchschnittliche Success Rate über alle 4 Runden
    avg_success_rate = rowMeans(select(., starts_with("round") & ends_with("success_rate")), na.rm = TRUE),
    
    # Durchschnittliche Response Time über alle 4 Runden
    avg_response_time = rowMeans(select(., starts_with("round") & ends_with("avg_time")), na.rm = TRUE),
    
    # Gesamtanzahl TimeUps über alle 4 Runden
    total_timeups = rowSums(select(., starts_with("round") & ends_with("timeups")), na.rm = TRUE),
    
    # Gesamtanzahl Trials über alle 4 Runden
    total_trials = rowSums(select(., starts_with("round") & ends_with("trials")), na.rm = TRUE)
  )

# 8. Sauberen Datensatz für weitere Analysen erstellen - ERWEITERT
math_analysis_data <- final_math_performance %>%
  select(participant.code, team_id, condition, 
         # Schwierigkeitsgrad-Zuordnung (für Referenz)
         difficulty_round3, difficulty_round4, difficulty_round5, difficulty_round6,
         # Performance nach Schwierigkeitsgrad
         easy_success_rate, easy_trials, easy_timeups, easy_avg_time, easy_correct_trials,  # NEU: correct_trials
         optimal_selected_success_rate, optimal_selected_trials, optimal_selected_timeups, optimal_selected_avg_time, optimal_selected_correct_trials,  # NEU
         optimal_calibrated_success_rate, optimal_calibrated_trials, optimal_calibrated_timeups, optimal_calibrated_avg_time, optimal_calibrated_correct_trials,  # NEU
         hard_success_rate, hard_trials, hard_timeups, hard_avg_time, hard_correct_trials)  # NEU

# 9. Schwierigkeitsgrad-basierte Analysen und Überblick
cat("=== MATH PERFORMANCE BY DIFFICULTY ===\n")
cat("Anzahl Teams:", nrow(math_analysis_data), "\n")

# Getrennte Analysen pro Condition
jitsi_teams <- math_analysis_data %>% filter(condition == "jitsi")
chat_teams <- math_analysis_data %>% filter(condition == "chat")

cat("Jitsi Teams:", nrow(jitsi_teams), "\n")
cat("Chat Teams:", nrow(chat_teams), "\n")

# Performance nach Schwierigkeitsgrad
difficulties <- c("easy", "optimal_selected", "optimal_calibrated", "hard")

cat("\n=== SUCCESS RATES BY DIFFICULTY ===\n")
for (diff in difficulties) {
  success_var <- paste0(diff, "_success_rate")
  
  if (nrow(jitsi_teams) > 0) {
    jitsi_rate <- mean(jitsi_teams[[success_var]], na.rm = TRUE)
    cat(sprintf("%-20s Jitsi: %.3f", paste(diff, ":"), jitsi_rate), "\n")
  }
  
  if (nrow(chat_teams) > 0) {
    chat_rate <- mean(chat_teams[[success_var]], na.rm = TRUE)  
    cat(sprintf("%-20s Chat:  %.3f", "", chat_rate), "\n")
  }
  cat("\n")
}

# NEU: Correct Trials Counts
cat("=== DURCHSCHNITTLICHE ANZAHL KORREKTER TRIALS ===\n")
for (diff in difficulties) {
  correct_var <- paste0(diff, "_correct_trials")
  
  if (nrow(jitsi_teams) > 0) {
    jitsi_correct <- mean(jitsi_teams[[correct_var]], na.rm = TRUE)
    cat(sprintf("%-20s Jitsi: %.1f", paste(diff, ":"), jitsi_correct), "\n")
  }
  
  if (nrow(chat_teams) > 0) {
    chat_correct <- mean(chat_teams[[correct_var]], na.rm = TRUE)  
    cat(sprintf("%-20s Chat:  %.1f", "", chat_correct), "\n")
  }
  cat("\n")
}

cat("=== RESPONSE TIMES BY DIFFICULTY (ms) ===\n")
for (diff in difficulties) {
  time_var <- paste0(diff, "_avg_time")
  
  if (nrow(jitsi_teams) > 0) {
    jitsi_time <- mean(jitsi_teams[[time_var]], na.rm = TRUE)
    cat(sprintf("%-20s Jitsi: %.0f", paste(diff, ":"), jitsi_time), "\n")
  }
  
  if (nrow(chat_teams) > 0) {
    chat_time <- mean(chat_teams[[time_var]], na.rm = TRUE)
    cat(sprintf("%-20s Chat:  %.0f", "", chat_time), "\n")
  }
  cat("\n")
}

# Zeige ersten Datensatz
head(math_analysis_data)

# Optional: Speichere den Math-Datensatz
# write.csv(math_analysis_data, "math_performance_analysis.csv", row.names = FALSE)

# =============================================================================
# HIDDEN PROFILE ANALYSIS
# =============================================================================

# HP Parser-Funktion für choice-basierte Performance - FIXED für Listen
parse_hp_performance <- function(team_data_row, condition, hp_order) {
  # Korrekte Antworten für jedes Project - KORRIGIERT
  correct_answers <- list(
    project1 = "Project A: EcoTrack",     # MED/Optimal
    project2 = "Project B: InsightFlow",   # EASY
    project3 = "Project D: AuditAce"      # HARD
  )
  
  tryCatch({
    # Robuste Extraktion des hp_order Vektors
    if (is.list(hp_order) && length(hp_order) > 0) {
      hp_vector <- hp_order[[1]]  # Extrahiere den Vektor aus der Liste
    } else if (is.vector(hp_order)) {
      hp_vector <- hp_order
    } else {
      hp_vector <- as.character(hp_order)
    }
    
    round_results <- list()
    
    # Analysiere jede Runde (1-3)
    for (round in 1:3) {
      # Robuste Extraktion der Schwierigkeit
      if (length(hp_vector) >= round) {
        difficulty <- as.character(hp_vector[round])
        difficulty <- str_trim(difficulty)  # Entferne Whitespace
      } else {
        difficulty <- NA_character_
      }
      
      # Mappe Schwierigkeit zu Project
      project_num <- case_when(
        difficulty == "EASY" ~ 2,
        difficulty == "MED" ~ 1, 
        difficulty == "HARD" ~ 3,
        TRUE ~ NA_real_
      ...
      if (!is.na(project_num)) {
        # Erstelle Variablennamen für diese Runde
        var_name <- if (condition == "jitsi") {
          paste0("HiddenProfile_Jitsi.", round, ".player.project", project_num, "_choice")
        } else {
          paste0("HiddenProfile_Chat.", round, ".player.project", project_num, "_choice")
        }
        
        # Hole den Wert
        team_choice <- team_data_row[[var_name]]
        correct_answer <- correct_answers[[paste0("project", project_num)]]
        
        # Vereinfachte Punktevergabe (später erweitern für alle 3 Spieler)
        points <- if (!is.na(team_choice) && !is.null(team_choice) && team_choice != "") {
          if (team_choice == correct_answer) {
            15  # Korrekte Antwort
          } else {
            10  # Falsche aber einheitliche Antwort (vereinfacht)
          }
        } else {
          0  # Keine Antwort
        }
        
        round_results[[paste0("round", round, "_points")]] <- points
        round_results[[paste0("round", round, "_difficulty")]] <- difficulty
        round_results[[paste0("round", round, "_choice")]] <- team_choice
        round_results[[paste0("round", round, "_correct")]] <- correct_answer
        round_results[[paste0("round", round, "_variable")]] <- var_name
      } else {
        round_results[[paste0("round", round, "_points")]] <- NA
        round_results[[paste0("round", round, "_difficulty")]] <- difficulty
        round_results[[paste0("round", round, "_variable")]] <- NA
        round_results[[paste0("round", round, "_choice")]] <- NA
        round_results[[paste0("round", round, "_correct")]] <- NA
      }
    }
    
    # Mappe Punkte zu Schwierigkeitsgraden
    easy_points <- NA
    optimal_points <- NA 
    hard_points <- NA
    
    for (round in 1:3) {
      difficulty <- round_results[[paste0("round", round, "_difficulty")]]
      points <- round_results[[paste0("round", round, "_points")]]
      
      if (!is.na(difficulty)) {
        if (difficulty == "EASY") easy_points <- points
        else if (difficulty == "MED") optimal_points <- points
        else if (difficulty == "HARD") hard_points <- points
      }
    }
    
    return(list(
      round1_points = round_results$round1_points,
      round2_points = round_results$round2_points, 
      round3_points = round_results$round3_points,
      round1_variable = round_results$round1_variable,
      round2_variable = round_results$round2_variable,
      round3_variable = round_results$round3_variable,
      round1_choice = round_results$round1_choice,
      round2_choice = round_results$round2_choice,
      round3_choice = round_results$round3_choice,
    ...
    cat("ERROR in parse_hp_performance:", e$message, "\n")
    return(list(
      round1_points = NA, round2_points = NA, round3_points = NA,
      round1_variable = NA, round2_variable = NA, round3_variable = NA,
      round1_choice = NA, round2_choice = NA, round3_choice = NA,
      easy_points = NA, optimal_points = NA, hard_points = NA
    ))
  })
}

# =============================================================================
# VOLLSTÄNDIGE HIDDEN PROFILE ANALYSE FÜR ALLE TEAMS
# =============================================================================

# HP Daten für alle Teams erstellen
hp_data <- team_data %>%
  select(participant.code, team_id, hp_order, 
         starts_with("HiddenProfile")) %>%
  # Condition identifizieren (gleich wie bei Math)
  mutate(
    has_jitsi = !is.na(team_data$mathJitsi.3.player.action_log) & team_data$mathJitsi.3.player.action_log != "",
    has_chat = !is.na(team_data$mathChat.3.player.action_log) & team_data$mathChat.3.player.action_log != "",
    condition = case_when(
      has_jitsi ~ "jitsi",
      has_chat ~ "chat", 
      TRUE ~ "unknown"
  ...
  # HP Performance für alle Teams berechnen
  rowwise() %>%
  mutate(
    hp_performance = list(parse_hp_performance(cur_data(), condition, hp_order))
  ) %>%
  ungroup()

# Extrahiere HP Performance Metriken
extract_hp_metrics <- function(df) {
  df %>%
    mutate(
      # Performance pro Runde
      hp_round1_points = sapply(hp_performance, function(x) x$round1_points),
      hp_round2_points = sapply(hp_performance, function(x) x$round2_points),
      hp_round3_points = sapply(hp_performance, function(x) x$round3_points),
      
      # Performance pro Schwierigkeitsgrad
      hp_easy_points = sapply(hp_performance, function(x) x$easy_points),
      hp_optimal_points = sapply(hp_performance, function(x) x$optimal_points),
      hp_hard_points = sapply(hp_performance, function(x) x$hard_points)
    ) %>%
    # Entferne list-columns
    select(-hp_performance)
}

# Anwenden der HP Metrik-Extraktion
final_hp_performance <- extract_hp_metrics(hp_data)

# Erstelle HP Analysis Datensatz
hp_analysis_data <- final_hp_performance %>%
  select(participant.code, team_id, condition,
         hp_round1_points, hp_round2_points, hp_round3_points,
         hp_easy_points, hp_optimal_points, hp_hard_points)

# HP Performance Analyse
cat("=== HIDDEN PROFILE PERFORMANCE ANALYSIS ===\n")
cat("Anzahl Teams:", nrow(hp_analysis_data), "\n")

# Getrennte Analysen pro Condition
jitsi_teams_hp <- hp_analysis_data %>% filter(condition == "jitsi")
chat_teams_hp <- hp_analysis_data %>% filter(condition == "chat")

cat("Jitsi Teams:", nrow(jitsi_teams_hp), "\n")
cat("Chat Teams:", nrow(chat_teams_hp), "\n")

# Performance nach Schwierigkeitsgrad
hp_difficulties <- c("easy", "optimal", "hard")

cat("\n=== AVERAGE POINTS BY DIFFICULTY (HP) ===\n")
for (diff in hp_difficulties) {
  points_var <- paste0("hp_", diff, "_points")
  
  if (nrow(jitsi_teams_hp) > 0) {
    jitsi_avg <- mean(jitsi_teams_hp[[points_var]], na.rm = TRUE)
    cat(sprintf("%-10s Jitsi: %.1f", paste(diff, ":"), jitsi_avg), "\n")
  }
  
  if (nrow(chat_teams_hp) > 0) {
    chat_avg <- mean(chat_teams_hp[[points_var]], na.rm = TRUE)  
    cat(sprintf("%-10s Chat:  %.1f", "", chat_avg), "\n")
  }
  cat("\n")
}

# =============================================================================
# ERWEITERTE HP ANALYSE: DISKUSSIONSZEITEN & NEXT-BUTTON
# =============================================================================

# Erweiterte HP Parser-Funktion mit Zeitanalyse
parse_hp_extended <- function(team_data_row, condition, hp_order) {
  # Basis HP Performance
  base_performance <- parse_hp_performance(team_data_row, condition, hp_order)
  
  # Zeit-Analyse für jede Runde
  time_analysis <- list()
  
  tryCatch({
    # Robuste hp_order Extraktion
    if (is.list(hp_order) && length(hp_order) > 0) {
      hp_vector <- hp_order[[1]]
    } else {
      hp_vector <- hp_order
    }
    
    for (round in 1:3) {
      # Zeit-Variablen für diese Runde - ZURÜCK ZU DISCUSSION
      load_time_var <- if (condition == "jitsi") {
        paste0("HiddenProfile_Jitsi.", round, ".player.task_load_time_discussion")
      } else {
        paste0("HiddenProfile_Chat.", round, ".player.task_load_time_discussion") 
      }
      
      end_time_var <- if (condition == "jitsi") {
        paste0("HiddenProfile_Jitsi.", round, ".player.task_end_time_discussion")
      } else {
        paste0("HiddenProfile_Chat.", round, ".player.task_end_time_discussion")
      }
      
      # Next-Button Variable (falls vorhanden)
      next_button_var <- if (condition == "jitsi") {
        paste0("HiddenProfile_Jitsi.", round, ".player.next_ready")
      } else {
        paste0("HiddenProfile_Chat.", round, ".player.next_ready")
      }
      
      # Hole Werte
      load_time_raw <- team_data_row[[load_time_var]]
      end_time_raw <- team_data_row[[end_time_var]]
      next_button_raw <- team_data_row[[next_button_var]]
      
      # Konvertiere zu numerischen Werten
      load_time <- as.numeric(if (is.list(load_time_raw)) unlist(load_time_raw)[1] else load_time_raw)
      end_time <- as.numeric(if (is.list(end_time_raw)) unlist(end_time_raw)[1] else end_time_raw)
      next_button <- if (!is.null(next_button_raw) && !is.na(next_button_raw)) {
        as.logical(if (is.list(next_button_raw)) unlist(next_button_raw)[1] else next_button_raw)
      } else FALSE
      
      # Berechne Diskussionszeit (in Millisekunden)
      discussion_time <- if (!is.na(load_time) && !is.na(end_time) && end_time > load_time) {
        end_time - load_time
      } else NA
      
      # Schwierigkeit für diese Runde
      difficulty <- if (length(hp_vector) >= round) {
        str_trim(as.character(hp_vector[round]))
      } else NA
      
      time_analysis[[paste0("round", round, "_load_time")]] <- load_time
      time_analysis[[paste0("round", round, "_end_time")]] <- end_time
      time_analysis[[paste0("round", round, "_discussion_time")]] <- discussion_time
      time_analysis[[paste0("round", round, "_next_button")]] <- next_button
      time_analysis[[paste0("round", round, "_difficulty")]] <- difficulty
    }
    
    # Mappe zu Schwierigkeitsgraden
    easy_discussion_time <- NA
    optimal_discussion_time <- NA
    hard_discussion_time <- NA
    easy_next_button <- NA
    optimal_next_button <- NA
    hard_next_button <- NA
    
    for (round in 1:3) {
      difficulty <- time_analysis[[paste0("round", round, "_difficulty")]]
      discussion_time <- time_analysis[[paste0("round", round, "_discussion_time")]]
      next_button <- time_analysis[[paste0("round", round, "_next_button")]]
      
      if (!is.na(difficulty)) {
        if (difficulty == "EASY") {
          easy_discussion_time <- discussion_time
          easy_next_button <- next_button
        } else if (difficulty == "MED") {
          optimal_discussion_time <- discussion_time
          optimal_next_button <- next_button
        } else if (difficulty == "HARD") {
          hard_discussion_time <- discussion_time
          hard_next_button <- next_button
        }
      }
    }
    
    # Kombiniere mit base performance
    extended_performance <- c(base_performance, time_analysis, list(
      easy_discussion_time = easy_discussion_time,
      optimal_discussion_time = optimal_discussion_time,
      hard_discussion_time = hard_discussion_time,
      easy_next_button = easy_next_button,
      optimal_next_button = optimal_next_button,
      hard_next_button = hard_next_button
    ))
    
    return(extended_performance)
    
  }, error = function(e) {
    cat("ERROR in time analysis:", e$message, "\n")
    return(base_performance)
  })
}

# TESTE die erweiterte HP Analyse
cat("=== HP ZEIT-ANALYSE TEST ===\n")

sample_team <- team_data[1, ]
sample_condition <- if (!is.na(sample_team$mathJitsi.3.player.action_log) && sample_team$mathJitsi.3.player.action_log != "") "jitsi" else "chat"
sample_hp_order <- sample_team$hp_order

# Schaue verfügbare Zeit-Variablen an
time_vars <- names(team_data)[str_detect(names(team_data), "task_.*time|next_ready")]
cat("Verfügbare Zeit-Variablen (erste 10):\n")
print(head(time_vars, 10))

# DEBUG: Schaue nach DISCUSSION Zeit-Variablen speziell
cat("=== DISCUSSION ZEIT-VARIABLEN SUCHE ===\n")
discussion_vars <- names(team_data)[str_detect(names(team_data), "discussion")]
cat("Gefundene Discussion-Variablen:\n")
print(discussion_vars)

# Teste für eine Runde die Zeit-Werte - DISCUSSION VARIABLEN
for (round in 1:3) {
  load_var <- paste0("HiddenProfile_Jitsi.", round, ".player.task_load_time_discussion")
  end_var <- paste0("HiddenProfile_Jitsi.", round, ".player.task_end_time_discussion")
  next_var <- paste0("HiddenProfile_Jitsi.", round, ".player.next_ready")
  
  cat("Runde", round, ":\n")
  cat("  Suche Variable:", load_var, "\n")
  cat("  Existiert:", load_var %in% names(team_data), "\n")
  cat("  Suche Variable:", end_var, "\n") 
  cat("  Existiert:", end_var %in% names(team_data), "\n")
  
  if (load_var %in% names(team_data) && end_var %in% names(team_data)) {
    load_val <- sample_team[[load_var]]
    end_val <- sample_team[[end_var]]
    next_val <- sample_team[[next_var]]
    
    cat("  Load time:", toString(load_val), "\n")
    cat("  End time:", toString(end_val), "\n") 
    cat("  Next button:", toString(next_val), "\n")
    
    # Konvertiere und berechne
    load_num <- as.numeric(if (is.list(load_val)) unlist(load_val)[1] else load_val)
    end_num <- as.numeric(if (is.list(end_val)) unlist(end_val)[1] else end_val)
    
    if (!is.na(load_num) && !is.na(end_num)) {
      diff_ms <- end_num - load_num
      diff_sec <- diff_ms / 1000
      cat("  Diskussionszeit:", diff_ms, "ms =", round(diff_sec, 1), "Sekunden\n")
    }
  } else {
    cat("  PROBLEM: Variablen existieren nicht!\n")
  }
  cat("\n")
}

cat("=== JETZT VOLLSTÄNDIGE ANALYSE FÜR ALLE TEAMS ===\n")

# HP Daten mit Zeitanalyse für alle Teams erstellen
hp_data_extended <- team_data %>%
  select(participant.code, team_id, hp_order, 
         starts_with("HiddenProfile")) %>%
  # Condition identifizieren
  mutate(
    has_jitsi = !is.na(team_data$mathJitsi.3.player.action_log) & team_data$mathJitsi.3.player.action_log != "",
    has_chat = !is.na(team_data$mathChat.3.player.action_log) & team_data$mathChat.3.player.action_log != "",
    condition = case_when(
  ...
  # Erweiterte HP Performance für alle Teams berechnen
  rowwise() %>%
  mutate(
    hp_extended_performance = list(parse_hp_extended(cur_data(), condition, hp_order))
  ) %>%
  ungroup()

# Extrahiere erweiterte HP Performance Metriken - AKTUALISIERT
extract_hp_extended_metrics <- function(df) {
  df %>%
    mutate(
      # Basis Performance
      hp_easy_points = sapply(hp_extended_performance, function(x) x$easy_points),
      hp_optimal_points = sapply(hp_extended_performance, function(x) x$optimal_points),
      hp_hard_points = sapply(hp_extended_performance, function(x) x$hard_points),
      
      # NEU: Correct Answer Flags (TRUE/FALSE)
      hp_easy_correct = sapply(hp_extended_performance, function(x) {
        correct <- x$easy_is_correct
        if (!is.null(correct) && !is.na(correct)) as.logical(correct) else FALSE
      }),
      hp_optimal_correct = sapply(hp_extended_performance, function(x) {
        correct <- x$optimal_is_correct
        if (!is.null(correct) && !is.na(correct)) as.logical(correct) else FALSE
      }),
      hp_hard_correct = sapply(hp_extended_performance, function(x) {
        correct <- x$hard_is_correct
        if (!is.null(correct) && !is.na(correct)) as.logical(correct) else FALSE
      }),
      
      # NEU: Time to Answer (in Sekunden konvertiert)
      hp_easy_time_to_answer = sapply(hp_extended_performance, function(x) {
        time_ms <- x$easy_time_to_answer
        if (!is.null(time_ms) && !is.na(time_ms)) time_ms / 1000 else NA
      }),
      hp_optimal_time_to_answer = sapply(hp_extended_performance, function(x) {
        time_ms <- x$optimal_time_to_answer
        if (!is.null(time_ms) && !is.na(time_ms)) time_ms / 1000 else NA
      }),
      hp_hard_time_to_answer = sapply(hp_extended_performance, function(x) {
        time_ms <- x$hard_time_to_answer
        if (!is.null(time_ms) && !is.na(time_ms)) time_ms / 1000 else NA
      }),
      
      # Bestehende Diskussionszeiten (in Sekunden konvertiert)
      hp_easy_discussion_time = sapply(hp_extended_performance, function(x) {
        time_ms <- x$easy_discussion_time
        if (!is.null(time_ms) && !is.na(time_ms)) time_ms / 1000 else NA
      }),
      hp_optimal_discussion_time = sapply(hp_extended_performance, function(x) {
        time_ms <- x$optimal_discussion_time
        if (!is.null(time_ms) && !is.na(time_ms)) time_ms / 1000 else NA
      }),
      hp_hard_discussion_time = sapply(hp_extended_performance, function(x) {
        time_ms <- x$hard_discussion_time
        if (!is.null(time_ms) && !is.na(time_ms)) time_ms / 1000 else NA
      }),
      
      # Next-Button Nutzung
      hp_easy_next_button = sapply(hp_extended_performance, function(x) {
        btn <- x$easy_next_button
        if (!is.null(btn) && !is.na(btn)) as.logical(btn) else FALSE
      }),
      hp_optimal_next_button = sapply(hp_extended_performance, function(x) {
        btn <- x$optimal_next_button
        if (!is.null(btn) && !is.na(btn)) as.logical(btn) else FALSE
      }),
      hp_hard_next_button = sapply(hp_extended_performance, function(x) {
        btn <- x$hard_next_button
        if (!is.null(btn) && !is.na(btn)) as.logical(btn) else FALSE
      })
    ) %>%
    # Entferne list-columns
    select(-hp_extended_performance)
}

# Führe die Analyse aus
final_hp_extended <- extract_hp_extended_metrics(hp_data_extended)

# Erstelle finalen HP Analysis Datensatz
hp_complete_analysis <- final_hp_extended %>%
  select(participant.code, team_id, condition,
         hp_easy_points, hp_optimal_points, hp_hard_points,
         hp_easy_discussion_time, hp_optimal_discussion_time, hp_hard_discussion_time,
         hp_easy_next_button, hp_optimal_next_button, hp_hard_next_button)

cat("HP Complete Analysis erstellt! Anzahl Teams:", nrow(hp_complete_analysis), "\n")

# ERWEITERTE HP PERFORMANCE ANALYSE
cat("\n=== ERWEITERTE HP ANALYSE MIT ZEITMETRIKEN ===\n")

# Getrennte Analysen pro Condition
jitsi_teams_ext <- hp_complete_analysis %>% filter(condition == "jitsi")
chat_teams_ext <- hp_complete_analysis %>% filter(condition == "chat")

cat("Jitsi Teams:", nrow(jitsi_teams_ext), "\n")
cat("Chat Teams:", nrow(chat_teams_ext), "\n")

# Durchschnittliche Diskussionszeiten
cat("\n=== DURCHSCHNITTLICHE DISKUSSIONSZEITEN (SEKUNDEN) ===\n")
hp_difficulties <- c("easy", "optimal", "hard")

for (diff in hp_difficulties) {
  time_var <- paste0("hp_", diff, "_discussion_time")
  
  if (nrow(jitsi_teams_ext) > 0) {
    jitsi_avg_time <- mean(jitsi_teams_ext[[time_var]], na.rm = TRUE)
    cat(sprintf("%-10s Jitsi: %.1f sec", paste(diff, ":"), jitsi_avg_time), "\n")
  }
  
  if (nrow(chat_teams_ext) > 0) {
    chat_avg_time <- mean(chat_teams_ext[[time_var]], na.rm = TRUE)  
    cat(sprintf("%-10s Chat:  %.1f sec", "", chat_avg_time), "\n")
  }
  cat("\n")
}

# Next-Button Nutzungsraten
cat("=== NEXT-BUTTON NUTZUNGSRATEN (%) ===\n")
for (diff in hp_difficulties) {
  button_var <- paste0("hp_", diff, "_next_button")
  
  if (nrow(jitsi_teams_ext) > 0) {
    jitsi_button_rate <- mean(jitsi_teams_ext[[button_var]], na.rm = TRUE) * 100
    cat(sprintf("%-10s Jitsi: %.1f%%", paste(diff, ":"), jitsi_button_rate), "\n")
  }
  
  if (nrow(chat_teams_ext) > 0) {
    chat_button_rate <- mean(chat_teams_ext[[button_var]], na.rm = TRUE) * 100
    cat(sprintf("%-10s Chat:  %.1f%%", "", chat_button_rate), "\n")
  }
  cat("\n")
}

# VEREINFACHTE Correct Answer Rates basierend auf Punkten
cat("=== CORRECT ANSWER RATES (%) ===\n")
for (diff in hp_difficulties) {
  points_var <- paste0("hp_", diff, "_points")
  
  if (nrow(jitsi_teams_ext) > 0 && points_var %in% names(jitsi_teams_ext)) {
    # 15 Punkte = korrekt, alles andere = falsch
    jitsi_correct_count <- sum(jitsi_teams_ext[[points_var]] == 15, na.rm = TRUE)
    jitsi_total_count <- sum(!is.na(jitsi_teams_ext[[points_var]]))
    jitsi_correct_rate <- if (jitsi_total_count > 0) (jitsi_correct_count / jitsi_total_count) * 100 else 0
    cat(sprintf("%-10s Jitsi: %.1f%% (%d/%d)", paste(diff, ":"), jitsi_correct_rate, jitsi_correct_count, jitsi_total_count), "\n")
  } else {
    cat(sprintf("%-10s Jitsi: Variable nicht gefunden", paste(diff, ":")), "\n")
  }
  
  if (nrow(chat_teams_ext) > 0 && points_var %in% names(chat_teams_ext)) {
    chat_correct_count <- sum(chat_teams_ext[[points_var]] == 15, na.rm = TRUE)
    chat_total_count <- sum(!is.na(chat_teams_ext[[points_var]]))
    chat_correct_rate <- if (chat_total_count > 0) (chat_correct_count / chat_total_count) * 100 else 0
    cat(sprintf("%-10s Chat:  %.1f%% (%d/%d)", "", chat_correct_rate, chat_correct_count, chat_total_count), "\n")
  } else {
    cat(sprintf("%-10s Chat:  Variable nicht gefunden", ""), "\n")
  }
  cat("\n")
}

# Performance vs Zeit Korrelationen (für Jitsi Teams)
if (nrow(jitsi_teams_ext) > 0) {
  cat("=== PERFORMANCE vs DISKUSSIONSZEIT KORRELATIONEN (JITSI) ===\n")
  for (diff in hp_difficulties) {
    points_var <- paste0("hp_", diff, "_points")
    time_var <- paste0("hp_", diff, "_discussion_time")
    
    valid_data <- jitsi_teams_ext[!is.na(jitsi_teams_ext[[points_var]]) & !is.na(jitsi_teams_ext[[time_var]]), ]
    if (nrow(valid_data) > 2) {
      correlation <- cor(valid_data[[points_var]], valid_data[[time_var]])
      cat(sprintf("%-10s r = %.3f", paste(diff, ":"), correlation), "\n")
    }
  }
}

cat("\n=== HP COMPLETE ANALYSIS SAMPLE ===\n")
head(hp_complete_analysis)
...
# 1. Math Performance Datensatz exportieren
cat("Exportiere Math Performance Daten...\n")
write.csv(math_analysis_data, "math_performance_analysis.csv", row.names = FALSE)
cat("✓ math_performance_analysis.csv erstellt\n")

# 2. HP Performance Datensatz exportieren  
cat("Exportiere HP Performance Daten...\n")
write.csv(hp_complete_analysis, "hp_performance_analysis.csv", row.names = FALSE)
cat("✓ hp_performance_analysis.csv erstellt\n")

# 3. Kombinierter Datensatz (Math + HP zusammengeführt)
cat("Erstelle kombinierten Datensatz...\n")
combined_analysis <- merge(
  math_analysis_data, 
  hp_complete_analysis, 
  by = c("participant.code", "team_id", "condition"),
  all = TRUE
)

write.csv(combined_analysis, "combined_performance_analysis.csv", row.names = FALSE)
cat("✓ combined_performance_analysis.csv erstellt\n")

# 4. Summary-Statistiken exportieren
cat("Erstelle Summary-Statistiken...\n")

# Math Summary
math_summary <- data.frame(
  condition = rep(c("jitsi", "chat"), each = 4),
  difficulty = rep(c("easy", "optimal_selected", "optimal_calibrated", "hard"), 2),
  success_rate = c(
    mean(jitsi_teams$easy_success_rate, na.rm = TRUE),
    mean(jitsi_teams$optimal_selected_success_rate, na.rm = TRUE),
    mean(jitsi_teams$optimal_calibrated_success_rate, na.rm = TRUE),
    mean(jitsi_teams$hard_success_rate, na.rm = TRUE),
    mean(chat_teams$easy_success_rate, na.rm = TRUE),
    mean(chat_teams$optimal_selected_success_rate, na.rm = TRUE),
    mean(chat_teams$optimal_calibrated_success_rate, na.rm = TRUE),
    mean(chat_teams$hard_success_rate, na.rm = TRUE)
  ),
  avg_response_time = c(
    mean(jitsi_teams$easy_avg_time, na.rm = TRUE),
    mean(jitsi_teams$optimal_selected_avg_time, na.rm = TRUE),
    mean(jitsi_teams$optimal_calibrated_avg_time, na.rm = TRUE),
    mean(jitsi_teams$hard_avg_time, na.rm = TRUE),
    mean(chat_teams$easy_avg_time, na.rm = TRUE),
    mean(chat_teams$optimal_selected_avg_time, na.rm = TRUE),
    mean(chat_teams$optimal_calibrated_avg_time, na.rm = TRUE),
    mean(chat_teams$hard_avg_time, na.rm = TRUE)
  ),
  avg_correct_trials = c(
    mean(jitsi_teams$easy_correct_trials, na.rm = TRUE),
    mean(jitsi_teams$optimal_selected_correct_trials, na.rm = TRUE),
    mean(jitsi_teams$optimal_calibrated_correct_trials, na.rm = TRUE),
    mean(jitsi_teams$hard_correct_trials, na.rm = TRUE),
    mean(chat_teams$easy_correct_trials, na.rm = TRUE),
    mean(chat_teams$optimal_selected_correct_trials, na.rm = TRUE),
    mean(chat_teams$optimal_calibrated_correct_trials, na.rm = TRUE),
    mean(chat_teams$hard_correct_trials, na.rm = TRUE)
  ),
  task_type = "math"
)

# HP Summary
hp_difficulties_full <- c("easy", "optimal", "hard")
hp_summary <- data.frame(
  condition = rep(c("jitsi", "chat"), each = 3),
  difficulty = rep(hp_difficulties_full, 2),
  avg_points = c(
    mean(jitsi_teams_ext$hp_easy_points, na.rm = TRUE),
    mean(jitsi_teams_ext$hp_optimal_points, na.rm = TRUE),
    mean(jitsi_teams_ext$hp_hard_points, na.rm = TRUE),
    mean(chat_teams_ext$hp_easy_points, na.rm = TRUE),
    mean(chat_teams_ext$hp_optimal_points, na.rm = TRUE),
    mean(chat_teams_ext$hp_hard_points, na.rm = TRUE)
  ),
  correct_rate = c(
    sum(jitsi_teams_ext$hp_easy_points == 15, na.rm = TRUE) / sum(!is.na(jitsi_teams_ext$hp_easy_points)),
    sum(jitsi_teams_ext$hp_optimal_points == 15, na.rm = TRUE) / sum(!is.na(jitsi_teams_ext$hp_optimal_points)),
    sum(jitsi_teams_ext$hp_hard_points == 15, na.rm = TRUE) / sum(!is.na(jitsi_teams_ext$hp_hard_points)),
    sum(chat_teams_ext$hp_easy_points == 15, na.rm = TRUE) / sum(!is.na(chat_teams_ext$hp_easy_points)),
    sum(chat_teams_ext$hp_optimal_points == 15, na.rm = TRUE) / sum(!is.na(chat_teams_ext$hp_optimal_points)),
    sum(chat_teams_ext$hp_hard_points == 15, na.rm = TRUE) / sum(!is.na(chat_teams_ext$hp_hard_points))
  ),
  avg_discussion_time = c(
    mean(jitsi_teams_ext$hp_easy_discussion_time, na.rm = TRUE),
    mean(jitsi_teams_ext$hp_optimal_discussion_time, na.rm = TRUE),
    mean(jitsi_teams_ext$hp_hard_discussion_time, na.rm = TRUE),
    mean(chat_teams_ext$hp_easy_discussion_time, na.rm = TRUE),
    mean(chat_teams_ext$hp_optimal_discussion_time, na.rm = TRUE),
    mean(chat_teams_ext$hp_hard_discussion_time, na.rm = TRUE)
  ),
  next_button_rate = c(
    mean(jitsi_teams_ext$hp_easy_next_button, na.rm = TRUE),
    mean(jitsi_teams_ext$hp_optimal_next_button, na.rm = TRUE),
    mean(jitsi_teams_ext$hp_hard_next_button, na.rm = TRUE),
    mean(chat_teams_ext$hp_easy_next_button, na.rm = TRUE),
    mean(chat_teams_ext$hp_optimal_next_button, na.rm = TRUE),
    mean(chat_teams_ext$hp_hard_next_button, na.rm = TRUE)
  ),
  task_type = "hp"
)

write.csv(math_summary, "math_summary_statistics.csv", row.names = FALSE)
write.csv(hp_summary, "hp_summary_statistics.csv", row.names = FALSE)
cat("✓ math_summary_statistics.csv erstellt\n")
cat("✓ hp_summary_statistics.csv erstellt\n")

# 5. Übersicht der exportierten Dateien
cat("\n=== EXPORTIERTE DATEIEN ===\n")
exported_files <- c(
  "math_performance_analysis.csv",
  "hp_performance_analysis.csv", 
  "combined_performance_analysis.csv",
  "math_summary_statistics.csv",
  "hp_summary_statistics.csv"
)
...
for (file in exported_files) {
  if (file.exists(file)) {
    ...
    cat("✗", file, "(nicht erstellt)\n")
  }
}

cat("\nAlle Dateien wurden erfolgreich exportiert!\n")
cat("Die Dateien enthalten:\n")
cat("• math_performance_analysis.csv: Vollständige Math-Daten pro Team\n")
cat("• hp_performance_analysis.csv: Vollständige HP-Daten pro Team\n") 
cat("• combined_performance_analysis.csv: Math + HP kombiniert\n")
cat("• math_summary_statistics.csv: Zusammenfassung Math-Performance\n")
cat("• hp_summary_statistics.csv: Zusammenfassung HP-Performance\n")

# =============================================================================
# ANALYSE DES ALTEN DATENSATZES (MATH ONLY, MP vs SP)
# =============================================================================

cat("\n", rep("=", 80), "\n", sep="")
cat("ANALYSE DES ALTEN DATENSATZES (MATH ONLY, MP vs SP)\n")
cat(rep("=", 80), "\n", sep="")

# FUNKTION: Lade und analysiere den alten Datensatz - ROBUSTE VERSION
analyze_old_dataset <- function(data_old) {
  
  cat("=== ALTER DATENSATZ - DATENSTRUKTUR ===\n")
  cat("Anzahl Zeilen:", nrow(data_old), "\n")
  cat("Anzahl Sessions:", length(unique(data_old$SessionID)), "\n")
  cat("Conditions:", paste(unique(data_old$Condition), collapse=", "), "\n")
  cat("Treatments:", paste(unique(data_old$Treatment), collapse=", "), "\n")
  ...
  tryCatch({
    # Datenaufbereitung ohne pivot_wider
    old_analysis_data <- data_old %>%
      # Mappe Conditions zu Schwierigkeitsgraden
      mutate(
        difficulty = case_when(
          Condition == "MATH_BOREDOM" ~ "Easy",
          Condition == "MATH_AUTONOMY" ~ "Optimal_Selected", 
          Condition == "MATH_FLOW" ~ "Optimal_Calibrated",
          Condition == "MATH_OVERLOAD" ~ "Hard",
          TRUE ~ "Unknown"
        ),
        # Behandle Treatment als condition
        condition = case_when(
          Treatment == "MP" ~ "multiplayer",
          Treatment == "SP" ~ "singleplayer", 
          TRUE ~ "unknown"
        ),
        # Response time 
        response_time_ms = TrialDurationMean
      )
    
    cat("\nNach Datenaufbereitung:\n")
    print(head(old_analysis_data))
    
    # Manuell zu Wide-Format konvertieren (ohne pivot_wider)
    sessions <- unique(old_analysis_data$SessionID)
    
    old_team_data <- data.frame()
    
    for (session in sessions) {
      session_data <- old_analysis_data[old_analysis_data$SessionID == session, ]
      
      if (nrow(session_data) > 0) {
        # Erstelle eine Zeile für dieses Team
        team_row <- data.frame(
          team_id = session,
          condition = session_data$condition[1]  # Sollte für alle gleich sein
        )
        
        # Extrahiere Werte für jede Schwierigkeit
        for (diff in c("Easy", "Optimal_Selected", "Optimal_Calibrated", "Hard")) {
          diff_data <- session_data[session_data$difficulty == diff, ]
          
          if (nrow(diff_data) > 0) {
            team_row[[paste0(tolower(gsub("_", "_", diff)), "_success_rate")]] <- diff_data$PctTrialsCorrect[1]
            team_row[[paste0(tolower(gsub("_", "_", diff)), "_correct_trials")]] <- diff_data$NrTrialsCorrect[1]
            team_row[[paste0(tolower(gsub("_", "_", diff)), "_avg_time")]] <- diff_data$response_time_ms[1]
          } else {
            team_row[[paste0(tolower(gsub("_", "_", diff)), "_success_rate")]] <- NA
            team_row[[paste0(tolower(gsub("_", "_", diff)), "_correct_trials")]] <- NA
            team_row[[paste0(tolower(gsub("_", "_", diff)), "_avg_time")]] <- NA
          }
        }
        
        old_team_data <- rbind(old_team_data, team_row)
      }
    }
    
    cat("\nTeam-level Daten erstellt. Anzahl Teams:", nrow(old_team_data), "\n")
    
    # Analysiere pro Treatment
    mp_teams <- old_team_data[old_team_data$condition == "multiplayer", ]
    sp_teams <- old_team_data[old_team_data$condition == "singleplayer", ]
    
    cat("\n=== ALTER DATENSATZ - PERFORMANCE ANALYSE ===\n")
    cat("Anzahl Teams:", nrow(old_team_data), "\n")
    cat("Multi-Player Teams:", nrow(mp_teams), "\n")
    cat("Single-Player Teams:", nrow(sp_teams), "\n")
    
    # Performance nach Schwierigkeitsgrad
    difficulties <- c("easy", "optimal_selected", "optimal_calibrated", "hard")
    
    cat("\n=== SUCCESS RATES BY DIFFICULTY (ALTER DATENSATZ) ===\n")
    for (diff in difficulties) {
      success_var <- paste0(diff, "_success_rate")
      
      if (nrow(mp_teams) > 0 && success_var %in% names(mp_teams)) {
        mp_rate <- mean(mp_teams[[success_var]], na.rm = TRUE)
        cat(sprintf("%-20s MP: %.3f", paste(diff, ":"), mp_rate), "\n")
      }
      
      if (nrow(sp_teams) > 0 && success_var %in% names(sp_teams)) {
        sp_rate <- mean(sp_teams[[success_var]], na.rm = TRUE)  
        cat(sprintf("%-20s SP: %.3f", "", sp_rate), "\n")
      }
      cat("\n")
    }
    
    cat("=== RESPONSE TIMES BY DIFFICULTY (ALTER DATENSATZ, ms) ===\n")
    for (diff in difficulties) {
      time_var <- paste0(diff, "_avg_time")
      
      if (nrow(mp_teams) > 0 && time_var %in% names(mp_teams)) {
        mp_time <- mean(mp_teams[[time_var]], na.rm = TRUE)
        cat(sprintf("%-20s MP: %.0f", paste(diff, ":"), mp_time), "\n")
      }
      
      if (nrow(sp_teams) > 0 && time_var %in% names(sp_teams)) {
        sp_time <- mean(sp_teams[[time_var]], na.rm = TRUE)
        cat(sprintf("%-20s SP: %.0f", "", sp_time), "\n")
      }
      cat("\n")
    }
    
    cat("=== DURCHSCHNITTLICHE ANZAHL KORREKTER TRIALS (ALTER DATENSATZ) ===\n")
    for (diff in difficulties) {
      correct_var <- paste0(diff, "_correct_trials")
      
      if (nrow(mp_teams) > 0 && correct_var %in% names(mp_teams)) {
        mp_correct <- mean(mp_teams[[correct_var]], na.rm = TRUE)
        cat(sprintf("%-20s MP: %.1f", paste(diff, ":"), mp_correct), "\n")
      }
      
      if (nrow(sp_teams) > 0 && correct_var %in% names(sp_teams)) {
        sp_correct <- mean(sp_teams[[correct_var]], na.rm = TRUE)  
        cat(sprintf("%-20s SP: %.1f", "", sp_correct), "\n")
      }
      cat("\n")
    }
    
    # Export
    cat("=== EXPORT ALTER DATENSATZ ===\n")
    write.csv(old_team_data, "old_math_performance_analysis.csv", row.names = FALSE)
    cat("✓ old_math_performance_analysis.csv erstellt\n")
    
    # Summary Statistics
    old_summary <- data.frame(
      condition = rep(c("multiplayer", "singleplayer"), each = 4),
      difficulty = rep(difficulties, 2),
      success_rate = c(
        mean(mp_teams$easy_success_rate, na.rm = TRUE),
        mean(mp_teams$optimal_selected_success_rate, na.rm = TRUE),
        mean(mp_teams$optimal_calibrated_success_rate, na.rm = TRUE),
        mean(mp_teams$hard_success_rate, na.rm = TRUE),
        mean(sp_teams$easy_success_rate, na.rm = TRUE),
        mean(sp_teams$optimal_selected_success_rate, na.rm = TRUE),
        mean(sp_teams$optimal_calibrated_success_rate, na.rm = TRUE),
        mean(sp_teams$hard_success_rate, na.rm = TRUE)
      ),
      avg_response_time = c(
        mean(mp_teams$easy_avg_time, na.rm = TRUE),
        mean(mp_teams$optimal_selected_avg_time, na.rm = TRUE),
        mean(mp_teams$optimal_calibrated_avg_time, na.rm = TRUE),
        mean(mp_teams$hard_avg_time, na.rm = TRUE),
        mean(sp_teams$easy_avg_time, na.rm = TRUE),
        mean(sp_teams$optimal_selected_avg_time, na.rm = TRUE),
        mean(sp_teams$optimal_calibrated_avg_time, na.rm = TRUE),
        mean(sp_teams$hard_avg_time, na.rm = TRUE)
      ),
      avg_correct_trials = c(
        mean(mp_teams$easy_correct_trials, na.rm = TRUE),
        mean(mp_teams$optimal_selected_correct_trials, na.rm = TRUE),
        mean(mp_teams$optimal_calibrated_correct_trials, na.rm = TRUE),
        mean(mp_teams$hard_correct_trials, na.rm = TRUE),
        mean(sp_teams$easy_correct_trials, na.rm = TRUE),
        mean(sp_teams$optimal_selected_correct_trials, na.rm = TRUE),
        mean(sp_teams$optimal_calibrated_correct_trials, na.rm = TRUE),
        mean(sp_teams$hard_correct_trials, na.rm = TRUE)
      )
    )
    
    write.csv(old_summary, "old_math_summary_statistics.csv", row.names = FALSE)
    cat("✓ old_math_summary_statistics.csv erstellt\n")
    
    cat("\n=== ALTER DATENSATZ SAMPLE ===\n")
    print(head(old_team_data))
    
    return(old_team_data)
    
  }, error = function(e) {
    cat("ERROR in analyze_old_dataset:", e$message, "\n")
    cat("Versuche direkte Analyse ohne Transformation...\n")
    
    # Fallback: Direkte Analyse der Long-Format Daten
    old_analysis_simple <- data_old %>%
      mutate(
        difficulty = case_when(
          Condition == "MATH_BOREDOM" ~ "Easy",
          Condition == "MATH_AUTONOMY" ~ "Optimal_Selected", 
    ...
    cat("\n=== DIREKTE ANALYSE (LONG FORMAT) ===\n")
    for (diff in c("Easy", "Optimal_Selected", "Optimal_Calibrated", "Hard")) {
      diff_data <- old_analysis_simple[old_analysis_simple$difficulty == diff, ]
      
      mp_data <- diff_data[diff_data$condition == "multiplayer", ]
      sp_data <- diff_data[diff_data$condition == "singleplayer", ]
      
      if (nrow(mp_data) > 0) {
        mp_rate <- mean(mp_data$PctTrialsCorrect, na.rm = TRUE)
        cat(sprintf("%-20s MP: %.3f", paste(tolower(diff), ":"), mp_rate), "\n")
      }
      
      if (nrow(sp_data) > 0) {
        sp_rate <- mean(sp_data$PctTrialsCorrect, na.rm = TRUE)
        cat(sprintf("%-20s SP: %.3f", "", sp_rate), "\n")
      }
      cat("\n")
    }
    
    return(old_analysis_simple)
  })
}

# VERWENDUNG:
# Lade deinen alten Datensatz und führe die Analyse aus:
# data_old <- read.csv("alter_datensatz.csv")  # Ersetze mit deinem Dateinamen
# old_results <- analyze_old_dataset(data_old)

cat("\n=== ANLEITUNG FÜR ALTEN DATENSATZ ===\n")
cat("Der Datensatz 'data_old_performance' wurde bereits eingelesen.\n")
cat("Führe jetzt die Analyse aus:\n\n")

# Führe die Analyse direkt aus, da der Datensatz bereits geladen ist
if (exists("data_old_performance")) {
  cat("=== STARTE ANALYSE DES ALTEN DATENSATZES ===\n")
  old_results <- analyze_old_dataset(data_old_performance)
  cat("✓ Analyse des alten Datensatzes abgeschlossen!\n")
} else {
  cat("HINWEIS: Datensatz 'data_old_performance' nicht gefunden.\n")
  cat("Falls der Datensatz anders heißt, verwende:\n")
  cat("old_results <- analyze_old_dataset(DEIN_DATENSATZ_NAME)\n")
}
```

Manipulation Check - Subjective & Objective Difficulty

```{r}
# =============================================================================
# ERWEITERTE MANIPULATION CHECKS FÜR EXPERIMENT 1 & 2
# Appendix A1 – Manipulation Checks & Task Performance Comparison
# =============================================================================
...
# =============================================================================
# A1.1 SUBJECTIVE & OBJECTIVE DIFFICULTY 
# =============================================================================

# Hilfsfunktionen (wie in deinem Code)
parse_order_string <- function(s) {
  s %>%
    str_remove_all("\\[|\\]|'|\"") %>%
    str_split(",\\s*") %>%
...
# Datenaufbereitung für Experiment 2
data_exp2 <- data %>%
  mutate(
    math_order = map(as.character(participant.condition_order), parse_order_string) %>%
                   map(~ map_chr(.x, map_math_difficulty)),
    ...
    # Condition identifizieren 
    condition = case_when(
      !is.na(mathJitsi.3.player.action_log) & mathJitsi.3.player.action_log != "" ~ "Video",
      !is.na(mathChat.3.player.action_log) & mathChat.3.player.action_log != "" ~ "Chat",
      TRUE ~ "Unknown"
    ),
...
# =============================================================================
# MATH TASK - SUBJECTIVE DIFFICULTY (Experiment 2)
# =============================================================================

math_difficulty_long_exp2 <- data_exp2 %>%
  select(participant.code, math_order, condition, experiment,
         # Nur csb1 (erstes Item = Perceived Difficulty)
         mathChat.3.player.csb1, mathChat.4.player.csb1, 
         mathChat.5.player.csb1, mathChat.6.player.csb1,
         mathJitsi.3.player.csb1, mathJitsi.4.player.csb1,
         mathJitsi.5.player.csb1, mathJitsi.6.player.csb1) %>%
  pivot_longer(cols = -c(participant.code, math_order, condition, experiment),
               names_to = "var", values_to = "subjective_difficulty") %>%
  mutate(
    round = str_extract(var, "\\d+"),
    comm_type = ifelse(str_detect(var, "Chat"), "Chat", "Video"),
    index = as.integer(round) - 2  # round 3 = index 1
  ) %>%
  filter(comm_type == condition) %>%  # Nur relevante Condition
  drop_na(subjective_difficulty) %>%
  rowwise() %>%
  mutate(
    # KONSISTENTE METHODE: unlist(math_order) 
    difficulty = tryCatch({
      math_vec <- unlist(math_order)
      if (length(math_vec) >= index) math_vec[index] else NA_character_
...
# =============================================================================
# HIDDEN PROFILE TASK - SUBJECTIVE DIFFICULTY (Experiment 2)
# =============================================================================

hp_difficulty_long_exp2 <- data_exp2 %>%
  select(participant.code, hp_order, condition, experiment,
         HiddenProfile_Chat.1.player.csb1, HiddenProfile_Chat.2.player.csb1, HiddenProfile_Chat.3.player.csb1,
         HiddenProfile_Jitsi.1.player.csb1, HiddenProfile_Jitsi.2.player.csb1, HiddenProfile_Jitsi.3.player.csb1) %>%
  pivot_longer(cols = matches("HiddenProfile_.*\\.player\\.csb1$"),
               names_to = "var", values_to = "subjective_difficulty") %>%
  mutate(
    round = str_extract(var, "(?<=\\.)\\d+"),
    comm_type = ifelse(str_detect(var, "Chat"), "Chat", "Video"),
    index = as.integer(round)
  ) %>%
  filter(comm_type == condition) %>%
  drop_na(subjective_difficulty) %>%
  rowwise() %>%
  mutate(
    # KORRIGIERTE METHODE: unlist(hp_order)
    difficulty = tryCatch({
      hp_vec <- unlist(hp_order)
      if (length(hp_vec) >= index) hp_vec[index] else NA_character_
    }, error = function(e) NA_character_)
  ) %>%
  ungroup() %>%
  drop_na(difficulty) %>%
  mutate(
    # Normalisiere Schwierigkeitsnamen für HP
    difficulty = case_when(
      str_to_upper(difficulty) == "EASY" ~ "Easy",
      str_to_upper(difficulty) %in% c("MED", "MEDIUM") ~ "Medium", 
      str_to_upper(difficulty) == "HARD" ~ "Hard",
      TRUE ~ difficulty
    ),
...
# =============================================================================
# EXPERIMENT 1 DATEN VORBEREITEN 
# =============================================================================

# Subjective Difficulty (aus data_old_rounds - Long Format)
prepare_exp1_subjective <- function(data_old_rounds) {
  if (!exists("data_old_rounds")) {
    cat("Warnung: data_old_rounds nicht gefunden. Überspringe Experiment 1 subjective analysis.\n")
    return(NULL)
  }
  
  exp1_difficulty_long <- data_old_rounds %>%
    mutate(
      # Schwierigkeit direkt aus Condition Variable extrahieren
      difficulty = case_when(
        Condition == "1_EASY" ~ "Easy",
        Condition == "3_AUTO" ~ "Optimal_Selected", 
        Condition == "2_CAL" ~ "Optimal_Calibrated",
        Condition == "4_HARD" ~ "Hard",
        TRUE ~ "Unknown"
      ),
      # Team vs. Alone aus Treatment
      condition = case_when(
        Treatment == "MP" ~ "Team",
        Treatment == "SP" ~ "Alone", 
        TRUE ~ "Unknown"
    ...
    dplyr::rename(
      participant.code = SessionID,
      subjective_difficulty = iDifficulty
    ) %>%
    select(participant.code, difficulty, condition, experiment, task_type, subjective_difficulty) %>%
    drop_na(subjective_difficulty) %>%
    filter(difficulty != "Unknown")  # Entferne unbekannte Schwierigkeitsgrade
  
  return(exp1_difficulty_long)
}

# Objective Difficulty (aus data_old_performance - Wide Format)
prepare_exp1_objective <- function(data_old_performance) {
  if (!exists("data_old_performance")) {
    cat("Warnung: data_old_performance nicht gefunden. Überspringe Experiment 1 objective analysis.\n")
    return(NULL)
  }
  
  exp1_objective_long <- data_old_performance %>%
    mutate(
      # Schwierigkeit aus Condition Variable - KORRIGIERTE KODIERUNG für data_old_performance
      difficulty = case_when(
        Condition == "MATH_BOREDOM" ~ "Easy",           # Easy
        Condition == "MATH_AUTONOMY" ~ "Optimal_Selected",  # Optimal Selected
        Condition == "MATH_FLOW" ~ "Optimal_Calibrated",    # Optimal Calibrated  
        Condition == "MATH_OVERLOAD" ~ "Hard",          # Hard
        TRUE ~ paste("Unknown:", Condition)
      ),
      condition = case_when(
        Treatment == "MP" ~ "Team",
        Treatment == "SP" ~ "Alone", 
      ...
      task_type = "Math",
      difficulty = factor(difficulty, levels = c("Easy", "Optimal_Selected", "Optimal_Calibrated", "Hard")),
      # Verwende NrTrialsCorrect als objective measure
      correct_trials = NrTrialsCorrect
    ) %>%
    dplyr::rename(participant.code = SessionID) %>%
    select(participant.code, difficulty, condition, experiment, task_type, correct_trials) %>%
    drop_na(correct_trials) %>%
    filter(!str_detect(difficulty, "Unknown"))  # Entferne unbekannte Schwierigkeitsgrade
  
  return(exp1_objective_long)
}

# Experiment 1 Daten laden (falls vorhanden)
if (exists("data_old_rounds")) {
  exp1_difficulty_long <- prepare_exp1_subjective(data_old_rounds)
  if (!is.null(exp1_difficulty_long)) {
    cat("✓ Experiment 1 subjective data geladen: ", nrow(exp1_difficulty_long), " Beobachtungen\n")
    cat("  Schwierigkeitsgrade: ", paste(unique(exp1_difficulty_long$difficulty), collapse = ", "), "\n")
  }
} else {
  exp1_difficulty_long <- NULL
  cat("Experiment 1 subjective data (data_old_rounds) nicht verfügbar\n")
}

if (exists("data_old_performance")) {
  exp1_objective_long <- prepare_exp1_objective(data_old_performance)
  if (!is.null(exp1_objective_long)) {
    cat("✓ Experiment 1 objective data geladen: ", nrow(exp1_objective_long), " Beobachtungen\n")
    cat("  Schwierigkeitsgrade: ", paste(unique(exp1_objective_long$difficulty), collapse = ", "), "\n")
    cat("  Conditions: ", paste(unique(exp1_objective_long$condition), collapse = ", "), "\n")
    
    # Debug: Prüfe ob alle Kombinationen vorhanden sind
    combo_check <- exp1_objective_long %>%
  ...
  exp1_objective_long <- NULL
  cat("Experiment 1 objective data (data_old_performance) nicht verfügbar\n")
}

# =============================================================================
# OBJECTIVE DIFFICULTY - PERFORMANCE METRIKEN
# =============================================================================

# Math Performance aus deiner Analyse verwenden
extract_math_objective_difficulty <- function(math_analysis_data) {
  math_analysis_data %>%
    select(participant.code, condition, 
           easy_correct_trials, optimal_selected_correct_trials, 
           optimal_calibrated_correct_trials, hard_correct_trials) %>%
    pivot_longer(cols = ends_with("_correct_trials"),
                 names_to = "difficulty_raw", 
                 values_to = "correct_trials") %>%
    mutate(
      difficulty = case_when(
        str_detect(difficulty_raw, "easy") ~ "Easy",
        str_detect(difficulty_raw, "optimal_selected") ~ "Optimal_Selected",
        str_detect(difficulty_raw, "optimal_calibrated") ~ "Optimal_Calibrated", 
        str_detect(difficulty_raw, "hard") ~ "Hard"
      ),
      difficulty = factor(difficulty, levels = c("Easy", "Optimal_Selected", "Optimal_Calibrated", "Hard")),
      condition = ifelse(condition == "jitsi", "Video", "Chat"),
      experiment = "Experiment_2",
      task_type = "Math"
    ) %>%
    drop_na(correct_trials)
...
# HP Performance extrahieren
extract_hp_objective_difficulty <- function(hp_complete_analysis) {
  hp_complete_analysis %>%
    select(participant.code, condition,
           hp_easy_points, hp_optimal_points, hp_hard_points) %>%
    ...
    mutate(
      difficulty = case_when(
        str_detect(difficulty_raw, "easy") ~ "Easy",
        str_detect(difficulty_raw, "optimal") ~ "Medium", 
        str_detect(difficulty_raw, "hard") ~ "Hard"
      ),
      difficulty = factor(difficulty, levels = c("Easy", "Medium", "Hard")),
      condition = ifelse(condition == "jitsi", "Video", "Chat"),
      experiment = "Experiment_2", 
      task_type = "Hidden_Profile",
      # Konvertiere Punkte zu Erfolgsrate (15 = korrekt, <15 = falsch)
      success_rate = ifelse(points == 15, 1, 0)
    ) %>%
    drop_na(points)
}

# Extrahiere objective difficulty data (verwende deine bereits erstellten Datensätze)
if (exists("math_analysis_data")) {
  math_objective_exp2 <- extract_math_objective_difficulty(math_analysis_data)
} else {
  math_objective_exp2 <- NULL
  cat("math_analysis_data nicht gefunden\n")
}

if (exists("hp_complete_analysis")) {
  hp_objective_exp2 <- extract_hp_objective_difficulty(hp_complete_analysis)
} else {
  hp_objective_exp2 <- NULL
  cat("hp_complete_analysis nicht gefunden\n")
}

# =============================================================================
# KOMBINIERTE MATH-MODELLE FÜR EXPERIMENT 1 & 2 - INTEGRIERT
# =============================================================================

# Diese Sektion ersetzt die separaten Exp1/Exp2 Math Analysen im bestehenden Code
# Füge nach der Datenaufbereitung, aber vor den ursprünglichen LMM Analysen ein

# Funktion zur Standardisierung und Kombination der Math Daten
create_combined_math_datasets <- function() {
  
  cat("\n=== KOMBINIERE MATH DATENSÄTZE ===\n")
  
  # SUBJECTIVE DIFFICULTY: Kombiniere und standardisiere
  combined_math_subjective <- NULL
  
  # Exp1 Daten hinzufügen (falls vorhanden)
  if (!is.null(exp1_difficulty_long)) {
    exp1_subj_clean <- exp1_difficulty_long %>%
      mutate(
        condition = case_when(
          condition == "Team" ~ "Together_None",
          condition == "Alone" ~ "Alone",
          TRUE ~ condition
        )
      ) %>%
      select(participant.code, difficulty, condition, task_type, subjective_difficulty)
    
    combined_math_subjective <- exp1_subj_clean
    cat("✓ Exp1 Subjective hinzugefügt:", nrow(exp1_subj_clean), "Beobachtungen\n")
  }
  
  # Exp2 Daten hinzufügen (falls vorhanden)
  if (!is.null(math_difficulty_long_exp2)) {
    exp2_subj_clean <- math_difficulty_long_exp2 %>%
      mutate(
        condition = case_when(
          condition == "Video" ~ "Together_Video",
          condition == "Chat" ~ "Together_Chat",
          TRUE ~ condition
        )
      ) %>%
      select(participant.code, difficulty, condition, task_type, subjective_difficulty)
    
    if (is.null(combined_math_subjective)) {
      combined_math_subjective <- exp2_subj_clean
    } else {
      combined_math_subjective <- bind_rows(combined_math_subjective, exp2_subj_clean)
    }
    cat("✓ Exp2 Subjective hinzugefügt:", nrow(exp2_subj_clean), "Beobachtungen\n")
  }
  
  # OBJECTIVE DIFFICULTY: Kombiniere und standardisiere
  combined_math_objective <- NULL
  
  # Exp1 Daten hinzufügen (falls vorhanden)
  if (!is.null(exp1_objective_long)) {
    exp1_obj_clean <- exp1_objective_long %>%
      mutate(
        condition = case_when(
          condition == "Team" ~ "Together_None",
          condition == "Alone" ~ "Alone", 
      ...
      select(participant.code, difficulty, condition, task_type, correct_trials)
    
    combined_math_objective <- exp1_obj_clean
    cat("✓ Exp1 Objective hinzugefügt:", nrow(exp1_obj_clean), "Beobachtungen\n")
  }
  
  # Exp2 Daten hinzufügen (falls vorhanden)
  if (!is.null(math_objective_exp2)) {
    exp2_obj_clean <- math_objective_exp2 %>%
      mutate(
        condition = case_when(
          condition == "Video" ~ "Together_Video",
          condition == "Chat" ~ "Together_Chat",
    ...
    if (is.null(combined_math_objective)) {
      combined_math_objective <- exp2_obj_clean
    } else {
      combined_math_objective <- bind_rows(combined_math_objective, exp2_obj_clean)
    }
    cat("✓ Exp2 Objective hinzugefügt:", nrow(exp2_obj_clean), "Beobachtungen\n")
  }
  
  # Faktor-Levels setzen
  if (!is.null(combined_math_subjective)) {
    combined_math_subjective <- combined_math_subjective %>%
      mutate(
        difficulty = factor(difficulty, levels = c("Easy", "Optimal_Selected", "Optimal_Calibrated", "Hard")),
        condition = factor(condition, levels = c("Alone", "Together_None", "Together_Chat", "Together_Video"))
      )
    cat("✓ Kombinierte Math Subjective:", nrow(combined_math_subjective), "Beobachtungen\n")
    cat("  Conditions:", paste(levels(combined_math_subjective$condition), collapse = ", "), "\n")
  }
  
  if (!is.null(combined_math_objective)) {
    combined_math_objective <- combined_math_objective %>%
      mutate(
        difficulty = factor(difficulty, levels = c("Easy", "Optimal_Selected", "Optimal_Calibrated", "Hard")),
        condition = factor(condition, levels = c("Alone", "Together_None", "Together_Chat", "Together_Video"))
      )
    cat("✓ Kombinierte Math Objective:", nrow(combined_math_objective), "Beobachtungen\n")
  }
  
  return(list(
    subjective = combined_math_subjective,
    objective = combined_math_objective
  ))
}

# Erstelle kombinierte Datensätze
combined_math_data <- create_combined_math_datasets()
combined_math_subjective <- combined_math_data$subjective
combined_math_objective <- combined_math_data$objective

# =============================================================================
# LMM ANALYSEN - SUBJECTIVE DIFFICULTY
# =============================================================================

run_subjective_difficulty_lmm <- function(data_long, title) {
  cat("\n=== LMM ANALYSIS:", title, "===\n")
  
  # Prüfe auf Singular Fit mit vollem Modell
  tryCatch({
    full_model <- lmer(subjective_difficulty ~ difficulty * condition + (1|participant.code), 
                       data = data_long, REML = FALSE)
    
    # Check for singular fit
    if (isSingular(full_model)) {
      cat("Singular fit detected - removing random effects\n")
      model <- lm(subjective_difficulty ~ difficulty * condition, data = data_long)
      anova_result <- anova(model)
    } else {
      model <- full_model
      anova_result <- anova(model)
    ...
    # Post-hoc tests if significant effects
    if (any(anova_result$`Pr(>F)` < 0.05, na.rm = TRUE) || 
        any(anova_result$`Pr(>Chisq)` < 0.05, na.rm = TRUE)) {
      cat("\n--- Post-hoc contrasts ---\n")
      emm <- emmeans(model, ~ difficulty | condition)
    ...
    cat("Error in LMM analysis:", e$message, "\n")
    return(NULL)
  })
}

# Führe Analysen durch
if (!is.null(math_difficulty_long_exp2) && nrow(math_difficulty_long_exp2) > 0) {
  math_subj_model_exp2 <- run_subjective_difficulty_lmm(math_difficulty_long_exp2, "MATH SUBJECTIVE DIFFICULTY - EXPERIMENT 2")
}

if (!is.null(hp_difficulty_long_exp2) && nrow(hp_difficulty_long_exp2) > 0) {
  # QUICK DEBUG: Was kommt wirklich an?
  cat("\n=== QUICK HP DEBUG ===\n")
  cat("Anzahl HP Beobachtungen:", nrow(hp_difficulty_long_exp2), "\n")
  cat("Anzahl unique Teilnehmer:", length(unique(hp_difficulty_long_exp2$participant.code)), "\n")
  
  hp_counts <- hp_difficulty_long_exp2 %>%
    group_by(participant.code) %>%
    summarise(n_obs = n(), .groups = "drop")
  
  cat("Beobachtungen pro Teilnehmer:\n")
  print(table(hp_counts$n_obs))
  
  # Wenn immer noch zu wenige, verwende linear model statt LMM
  if (length(unique(hp_difficulty_long_exp2$participant.code)) >= nrow(hp_difficulty_long_exp2)) {
    cat("Zu wenig Beobachtungen pro Teilnehmer - verwende linear model\n")
    
    # Direkte linear model Analyse
    tryCatch({
      lm_model <- lm(subjective_difficulty ~ difficulty * condition, data = hp_difficulty_long_exp2)
      cat("\n=== HP SUBJECTIVE DIFFICULTY - LINEAR MODEL ===\n")
      print(summary(lm_model))
      print(anova(lm_model))
      
      # Post-hoc tests
      library(emmeans)
      emm <- emmeans(lm_model, ~ difficulty | condition)
      cat("\n--- Post-hoc contrasts ---\n")
      print(pairs(emm, adjust = "bonferroni"))
      
      hp_subj_model_exp2 <- lm_model
      
    }, error = function(e) {
      cat("Auch linear model fehlgeschlagen:", e$message, "\n")
    })
    
  } else {
    hp_subj_model_exp2 <- run_subjective_difficulty_lmm(hp_difficulty_long_exp2, "HP SUBJECTIVE DIFFICULTY - EXPERIMENT 2")
  }
} else {
  cat("HP difficulty data nicht verfügbar oder leer\n")
}

if (!is.null(exp1_difficulty_long) && nrow(exp1_difficulty_long) > 0) {
  math_subj_model_exp1 <- run_subjective_difficulty_lmm(exp1_difficulty_long, "MATH SUBJECTIVE DIFFICULTY - EXPERIMENT 1")
}

# Kombinierte Math Subjective Difficulty Analysis
if (!is.null(combined_math_subjective)) {
  combined_math_subj_model <- run_subjective_difficulty_lmm(
    combined_math_subjective, 
    "MATH SUBJECTIVE DIFFICULTY - COMBINED EXPERIMENTS"
  )
}

# =============================================================================
# LMM ANALYSEN - OBJECTIVE DIFFICULTY  
# =============================================================================

run_objective_difficulty_lmm <- function(data_long, dv_var, title) {
  cat("\n=== LMM ANALYSIS:", title, "===\n")
  
  # Erstelle Formel basierend auf abhängiger Variable
  formula_str <- paste0(dv_var, " ~ difficulty * condition + (1|participant.code)")
  
  tryCatch({
    full_model <- lmer(as.formula(formula_str), data = data_long, REML = FALSE)
    
    if (isSingular(full_model)) {
      cat("Singular fit detected - removing random effects\n")
      formula_simple <- paste0(dv_var, " ~ difficulty * condition")
      model <- lm(as.formula(formula_simple), data = data_long)
      anova_result <- anova(model)
    } else {
      model <- full_model
      anova_result <- anova(model)
    ...
    cat("Error in objective difficulty LMM:", e$message, "\n")
    return(NULL)
  })
}

# Objective difficulty analyses
if (!is.null(math_objective_exp2) && nrow(math_objective_exp2) > 0) {
  math_obj_model_exp2 <- run_objective_difficulty_lmm(math_objective_exp2, "correct_trials", "MATH OBJECTIVE DIFFICULTY - EXPERIMENT 2")
}

if (!is.null(hp_objective_exp2) && nrow(hp_objective_exp2) > 0) {
  hp_obj_model_exp2 <- run_objective_difficulty_lmm(hp_objective_exp2, "success_rate", "HP OBJECTIVE DIFFICULTY - EXPERIMENT 2")
}

if (!is.null(exp1_objective_long) && nrow(exp1_objective_long) > 0) {
  math_obj_model_exp1 <- run_objective_difficulty_lmm(exp1_objective_long, "correct_trials", "MATH OBJECTIVE DIFFICULTY - EXPERIMENT 1")
}

# Kombinierte Math Objective Difficulty Analysis  
if (!is.null(combined_math_objective)) {
  combined_math_obj_model <- run_objective_difficulty_lmm(
    combined_math_objective, 
    "correct_trials",
    "MATH OBJECTIVE DIFFICULTY - COMBINED EXPERIMENTS"
  )
}
...
# Funktion für subjective difficulty plots
create_subjective_difficulty_plot <- function(data_long, title) {
  if (is.null(data_long) || nrow(data_long) == 0) {
    return(NULL)
  }
  
  ggplot(data_long, aes(x = difficulty, y = subjective_difficulty, fill = condition)) +
    ...
    labs(
      title = paste("Subjective Difficulty:", title),
      x = "Difficulty Condition", 
      y = "Subjective Difficulty Rating",
      fill = "Communication"
    ) +
...
# Funktion für objective difficulty plots
create_objective_difficulty_plot <- function(data_long, dv_var, y_label, title) {
  if (is.null(data_long) || nrow(data_long) == 0) {
    return(NULL)
  }
  
  ggplot(data_long, aes_string(x = "difficulty", y = dv_var, fill = "condition")) +
    geom_boxplot(position = position_dodge(width = 0.75), alpha = 0.7) +
    stat_summary(fun = mean, geom = "point", position = position_dodge(width = 0.75),
                 shape = 23, size = 3, color = "black") +
    labs(
      title = paste("Objective Difficulty:", title),
      x = "Difficulty Condition",
      y = y_label,
      fill = "Communication"
    ) +
...
# Funktion für HP objective difficulty plots (Barplot statt Boxplot)
create_hp_objective_plot <- function(data_long, title) {
  if (is.null(data_long) || nrow(data_long) == 0) {
    return(NULL)
  }
  
  # Berechne Success Rates pro Gruppe
  summary_data <- data_long %>%
    group_by(difficulty, condition) %>%
    summarise(
      success_rate = mean(success_rate, na.rm = TRUE),
      n = n(),
      se = sqrt(success_rate * (1 - success_rate) / n),  # Binomial SE
      .groups = "drop"
    )
  
  ggplot(summary_data, aes(x = difficulty, y = success_rate, fill = condition)) +
    geom_col(position = position_dodge(width = 0.8), alpha = 0.8) +
    geom_errorbar(aes(ymin = pmax(0, success_rate - se), 
                      ymax = pmin(1, success_rate + se)),
                  position = position_dodge(width = 0.8), 
    ...
    labs(
      title = paste("Objective Difficulty:", title),
      x = "Difficulty Condition",
      y = "Success Rate (Proportion Correct)",
      fill = "Communication"
    ) +
...
# Erstelle Plots
math_subj_plot_exp2 <- create_subjective_difficulty_plot(math_difficulty_long_exp2, "Math Task (Experiment 2)")
hp_subj_plot_exp2 <- create_subjective_difficulty_plot(hp_difficulty_long_exp2, "Hidden Profile Task (Experiment 2)")

if (!is.null(exp1_difficulty_long)) {
  math_subj_plot_exp1 <- create_subjective_difficulty_plot(exp1_difficulty_long, "Math Task (Experiment 1)")
} else {
  math_subj_plot_exp1 <- NULL
}

if (!is.null(math_objective_exp2)) {
  math_obj_plot_exp2 <- create_objective_difficulty_plot(math_objective_exp2, "correct_trials", 
                                                         "Number of Correct Trials", "Math Task (Experiment 2)")
} else {
  math_obj_plot_exp2 <- NULL
}

# HP: Verwende spezielle Barplot-Funktion für Success Rate
if (!is.null(hp_objective_exp2)) {
  hp_obj_plot_exp2 <- create_hp_objective_plot(hp_objective_exp2, "Hidden Profile Task (Experiment 2)")
} else {
  hp_obj_plot_exp2 <- NULL
}

if (!is.null(exp1_objective_long)) {
  math_obj_plot_exp1 <- create_objective_difficulty_plot(exp1_objective_long, "correct_trials", 
                                                         "Number of Correct Trials", "Math Task (Experiment 1)")
} else {
  math_obj_plot_exp1 <- NULL
}

# Zeige Plots an
if (!is.null(math_subj_plot_exp2)) print(math_subj_plot_exp2)
if (!is.null(hp_subj_plot_exp2)) print(hp_subj_plot_exp2)
if (!is.null(math_subj_plot_exp1)) print(math_subj_plot_exp1)
if (!is.null(math_obj_plot_exp2)) print(math_obj_plot_exp2)
if (!is.null(hp_obj_plot_exp2)) print(hp_obj_plot_exp2)
if (!is.null(math_obj_plot_exp1)) print(math_obj_plot_exp1)

# =============================================================================
# KOMBINIERTE MATH PLOTS MIT SCHÖNEN LABELS
# =============================================================================

create_combined_math_plot <- function(data_long, dv_var, y_label, title) {
  if (is.null(data_long) || nrow(data_long) == 0) {
    return(NULL)
  }
  
  # Erstelle schöne Labels für Plots
  data_plot <- data_long %>%
    mutate(
      condition_label = case_when(
        condition == "Alone" ~ "Alone",
        condition == "Together_None" ~ "No Communication", 
        condition == "Together_Chat" ~ "Chat",
        condition == "Together_Video" ~ "Video",
        TRUE ~ as.character(condition)
      ),
      condition_label = factor(condition_label, 
                              levels = c("Alone", "No Communication", "Chat", "Video"))
    )
  
  # Farben für die 4 Bedingungen - ANGEPASST
  colors <- c(
    "Alone" = "#E91E63",           # Rosarot
    "No Communication" = "#DDA0DD", # Helles Flieder
    "Chat" = "#87CEEB",            # Hellblau
    "Video" = "#1E3A8A"            # Dunkelblau
  )
  
  ggplot(data_plot, aes_string(x = "difficulty", y = dv_var, fill = "condition_label")) +
...
# Erstelle kombinierte Math Plots (ersetzen die separaten Exp1/Exp2 Math Plots)
if (!is.null(combined_math_subjective)) {
  combined_math_subj_plot <- create_combined_math_plot(
    combined_math_subjective, 
    "subjective_difficulty",
    "Subjective Difficulty Rating", 
    "Math Task: Subjective Difficulty"
  )
  
  print(combined_math_subj_plot)
}

if (!is.null(combined_math_objective)) {
  combined_math_obj_plot <- create_combined_math_plot(
    combined_math_objective,
    "correct_trials", 
    "Number of Correct Trials",
    "Math Task: Objective Difficulty"
  )
  
  print(combined_math_obj_plot)
}

# =============================================================================
# CROSS-TASK & CROSS-EXPERIMENT VERGLEICHE
# =============================================================================

# Funktionen für Vergleichsanalysen
compare_across_tasks_exp2 <- function() {
  if (!is.null(math_difficulty_long_exp2) && !is.null(hp_difficulty_long_exp2)) {
    cat("\n=== CROSS-TASK COMPARISON (EXPERIMENT 2) ===\n")
    
    # Normalisiere Schwierigkeitsgrade für Vergleich
    math_norm <- math_difficulty_long_exp2 %>%
      mutate(difficulty_norm = case_when(
        difficulty == "Easy" ~ "Easy",
        difficulty %in% c("Optimal_Selected", "Optimal_Calibrated") ~ "Medium",
        difficulty == "Hard" ~ "Hard"
      ))
    
    hp_norm <- hp_difficulty_long_exp2 %>%
      mutate(difficulty_norm = case_when(
        difficulty == "Easy" ~ "Easy", 
    ...
    # Kombiniere für Vergleich
    combined_exp2 <- bind_rows(
      math_norm %>% select(participant.code, condition, difficulty_norm, subjective_difficulty, task_type),
      hp_norm %>% select(participant.code, condition, difficulty_norm, subjective_difficulty, task_type)
    ) %>%
      mutate(difficulty_norm = factor(difficulty_norm, levels = c("Easy", "Medium", "Hard")))
    
    # Cross-task LMM
    if (nrow(combined_exp2) > 0) {
      cat("Running cross-task difficulty comparison...\n")
      tryCatch({
        cross_task_model <- lmer(subjective_difficulty ~ difficulty_norm * task_type * condition + 
                                (1|participant.code), data = combined_exp2, REML = FALSE)
        
        if (isSingular(cross_task_model)) {
          cross_task_model <- lm(subjective_difficulty ~ difficulty_norm * task_type * condition, 
                                data = combined_exp2)
        }
        
        print(summary(cross_task_model))
        print(anova(cross_task_model))
        
        # Plot für Cross-Task Vergleich
        cross_task_plot <- ggplot(combined_exp2, aes(x = difficulty_norm, y = subjective_difficulty, 
                                                     fill = interaction(task_type, condition))) +
          geom_boxplot(position = position_dodge(width = 0.8), alpha = 0.7) +
          labs(
            title = "Cross-Task Difficulty Comparison (Experiment 2)",
            x = "Normalized Difficulty", 
            y = "Subjective Difficulty Rating",
            fill = "Task × Communication"
          ) +
          theme_minimal() +
          theme(
            plot.title = element_text(hjust = 0.5, size = 14, face = "bold"),
        ...
        print(cross_task_plot)
        
        return(cross_task_model)
      }, error = function(e) {
        cat("Error in cross-task analysis:", e$message, "\n")
        return(NULL)
      })
    }
  }
...
# Führe Cross-Task Analyse durch
cross_task_results <- compare_across_tasks_exp2()

# =============================================================================
# 1. DIFFICULTY PLOTS EXPORTIEREN
# =============================================================================

cat("\n--- EXPORTIERE DIFFICULTY PLOTS ---\n")

# Plot-Dimensionen für verschiedene Zwecke
# Für Papers/Journals: kleinere Plots, höhere Auflösung
paper_width <- 7
paper_height <- 5
paper_dpi <- 600

# Für Präsentationen: größere Plots, mittlere Auflösung  
presentation_width <- 10
presentation_height <- 7
presentation_dpi <- 300

difficulty_plots_updated <- list(
  "combined_math_subj" = if(exists("combined_math_subj_plot")) combined_math_subj_plot else NULL,
  "combined_math_obj" = if(exists("combined_math_obj_plot")) combined_math_obj_plot else NULL,
  "hp_subj_exp2" = if(exists("hp_subj_plot_exp2")) hp_subj_plot_exp2 else NULL,
  "hp_obj_exp2" = if(exists("hp_obj_plot_exp2")) hp_obj_plot_exp2 else NULL
)

# Exportiere jeden Difficulty Plot
for (plot_name in names(difficulty_plots)) {
  plot_obj <- difficulty_plots[[plot_name]]
  
  if (!is.null(plot_obj)) {
    # Für Publikationen (TIFF, hochauflösend)
    tryCatch({
      ggsave(file.path("plots_export", paste0(plot_name, "_publication.tiff")), 
             plot = plot_obj,
             width = paper_width, height = paper_height, 
             dpi = paper_dpi, compression = "lzw", bg = "white")
      cat("✓ Exportiert:", paste0(plot_name, "_publication.tiff"), "\n")
    }, error = function(e) {
      cat("❌ Fehler beim Export von", plot_name, "TIFF:", e$message, "\n")
    })
    
    # Für Präsentationen (PNG, mittlere Auflösung)
    tryCatch({
      ggsave(file.path("plots_export", paste0(plot_name, "_presentation.png")), 
             plot = plot_obj,
             width = presentation_width, height = presentation_height, 
             dpi = presentation_dpi, bg = "white")
      cat("✓ Exportiert:", paste0(plot_name, "_presentation.png"), "\n")
    }, error = function(e) {
      cat("❌ Fehler beim Export von", plot_name, "PNG:", e$message, "\n")
    })
    
    # Für Vector Graphics (PDF)
    tryCatch({
      ggsave(file.path("plots_export", paste0(plot_name, "_vector.pdf")), 
             plot = plot_obj,
             width = paper_width, height = paper_height, 
             bg = "white")
      cat("✓ Exportiert:", paste0(plot_name, "_vector.pdf"), "\n")
    }, error = function(e) {
      cat("❌ Fehler beim Export von", plot_name, "PDF:", e$message, "\n")
    })
  } else {
    cat("⚠️ Plot", plot_name, "nicht verfügbar - übersprungen\n")
  }
}
...
cat("\n=== MANIPULATION CHECK ZUSAMMENFASSUNG ===\n")
cat("Abgeschlossene Analysen:\n")
if (exists("math_subj_model_exp2")) cat("✓ Math Subjective Difficulty - Experiment 2\n")
if (exists("hp_subj_model_exp2")) cat("✓ HP Subjective Difficulty - Experiment 2\n") 
if (exists("math_subj_model_exp1")) cat("✓ Math Subjective Difficulty - Experiment 1\n")
if (exists("math_obj_model_exp2")) cat("✓ Math Objective Difficulty - Experiment 2\n")
if (exists("hp_obj_model_exp2")) cat("✓ HP Objective Difficulty - Experiment 2\n")
if (exists("math_obj_model_exp1")) cat("✓ Math Objective Difficulty - Experiment 1\n")
if (exists("cross_task_results")) cat("✓ Cross-Task Comparison\n")

cat("\nErstellte Plots:\n")
if (exists("math_subj_plot_exp2")) cat("✓ Math Subjective - Experiment 2\n")
if (exists("hp_subj_plot_exp2")) cat("✓ HP Subjective - Experiment 2\n")
if (exists("math_subj_plot_exp1")) cat("✓ Math Subjective - Experiment 1\n")
if (exists("math_obj_plot_exp2")) cat("✓ Math Objective - Experiment 2\n")
if (exists("hp_obj_plot_exp2")) cat("✓ HP Objective - Experiment 2\n")
if (exists("math_obj_plot_exp1")) cat("✓ Math Objective - Experiment 1\n")

cat("\n=== AKTUALISIERTE DIFFICULTY ANALYSIS STRUKTUR ===\n")
cat("📊 MATH TASK (KOMBINIERT - Exp1 & Exp2):\n")
if (exists("combined_math_subj_model")) cat("✓ Subjective Difficulty - 4 Conditions kombiniert\n")
if (exists("combined_math_obj_model")) cat("✓ Objective Difficulty - 4 Conditions kombiniert\n")

cat("\n📋 HIDDEN PROFILE TASK (NUR EXP2):\n") 
if (exists("hp_subj_model_exp2")) cat("✓ Subjective Difficulty - Video vs Chat\n")
if (exists("hp_obj_model_exp2")) cat("✓ Objective Difficulty - Video vs Chat\n")

```

Manipulation Check - Team interaction format

```{r}
# =============================================================================
# A1.2 TEAM INTERACTION FORMAT ASSESSMENT - KORRIGIERT
# Appendix A1 – Manipulation Checks & Task Performance Comparison
# =============================================================================
...
# =============================================================================
# TEAM-FAKTOREN VARIABLEN & LABELS - KORRIGIERT
# =============================================================================

# Definiere Team-Faktoren Variablen (OHNE perceived_task_complexity_value)
team_factor_vars <- c(
  "interdependence_value", "common_goal_value", "means_coordination_value",
  "group_size_value", "group_diversity_value", "group_skill_value", 
  "team_composition_value", "team_motivation_value",
  "communication_required_value", "work_independence_value", 
  "social_presence_value"
  # "perceived_task_complexity_value" ENTFERNT
)

# Variable Labels für bessere Plots
variable_labels <- c(
  "interdependence_value" = "Mutual\nDependence",
  "common_goal_value" = "Clear Goal &\nProcess", 
  "means_coordination_value" = "Coordination\nPossible",
  "group_size_value" = "Group Size\nJust Right",
  "group_diversity_value" = "Adequate Perspective\nDiversity",
  "group_skill_value" = "Complementary\nSkills",
  "team_composition_value" = "Team Composition\nAdequate",
  "team_motivation_value" = "Team\nMotivation",
  "communication_required_value" = "Communication\nRequired",
  "work_independence_value" = "Work\nIndependence",
  "social_presence_value" = "Social\nPresence"
  # perceived_task_complexity_value entfernt
)

# =============================================================================
# DATENAUFBEREITUNG FÜR ALLE DREI TEIL-EXPERIMENTE - KORRIGIERT
# =============================================================================

prepare_all_experiments_data <- function(integrated_data_full) {
  
  cat("=== DATENAUFBEREITUNG FÜR ALLE DREI TEIL-EXPERIMENTE ===\n")
  
  if (!exists("integrated_data_full")) {
    cat("❌ integrated_data_full nicht gefunden\n")
    return(NULL)
  }
  
  # Team-Faktoren sind Post-Task Measurements
  all_experiments_data <- integrated_data_full %>%
    filter(
      # Nur Team-Settings (KORRIGIERT: Together_None statt MP)
      comm %in% c("Together_Chat", "Together_Jitsi", "Together_None"),
      # Nur Post-Task Measurements für Team-Faktoren
      round == "Post"
    ) %>%
    mutate(
      # Erstelle einheitliche Experiment-Task Kategorien (KORRIGIERT)
      experiment_task = case_when(
        comm == "Together_None" ~ "Exp1_Math_Team",  # KORRIGIERT
        comm == "Together_Chat" & task == "Math" ~ "Exp2_Math_Chat",
        comm == "Together_Chat" & task == "HP" ~ "Exp2_HP_Chat", 
        comm == "Together_Jitsi" & task == "Math" ~ "Exp2_Math_Video",
        comm == "Together_Jitsi" & task == "HP" ~ "Exp2_HP_Video",
        TRUE ~ "Unknown"
      ),
      # Vereinfachte Kategorien für Hauptvergleich
      experiment_simplified = case_when(
        comm == "Together_None" ~ "Exp1_Math_Team",  # KORRIGIERT
        comm %in% c("Together_Chat", "Together_Jitsi") & task == "Math" ~ "Exp2_Math_Team",
        comm %in% c("Together_Chat", "Together_Jitsi") & task == "HP" ~ "Exp2_HP_Team",
        TRUE ~ "Unknown"
      ),
      # Communication Type
      communication = case_when(
        comm == "Together_None" ~ "No_Medium",  # KORRIGIERT
        comm == "Together_Chat" ~ "Chat", 
        comm == "Together_Jitsi" ~ "Video",
        TRUE ~ "Unknown"
      )
    ) %>%
    filter(experiment_task != "Unknown") %>%
    select(participant.code, experiment_task, experiment_simplified, communication, 
           task, all_of(team_factor_vars))
  
  # Prüfe verfügbare Team-Variablen
  available_vars <- team_factor_vars[team_factor_vars %in% names(all_experiments_data)]
  
  cat("Gesamtdaten erstellt:\n")
  cat("Anzahl Teilnehmer:", nrow(all_experiments_data), "\n")
  cat("Experiment-Task Kombinationen:\n")
  print(table(all_experiments_data$experiment_task))
  cat("Vereinfachte Kategorien:\n")
  print(table(all_experiments_data$experiment_simplified))
  cat("Verfügbare Team-Variablen:", length(available_vars), "von", length(team_factor_vars), "\n")
  
  if (length(available_vars) == 0) {
    cat("❌ Keine Team-Variablen gefunden\n")
    return(NULL)
  }
  
  # Debug: Zeige verfügbare Variablen
  cat("Verfügbare Variablen:", paste(available_vars, collapse = ", "), "\n")
  
  return(list(
    data = all_experiments_data,
    available_vars = available_vars
  ))
}

# =============================================================================
# TEAM PROFILE PLOTS - DREI SEPARATE PLOTS FÜR BESSERE ÜBERSICHT - KORRIGIERT
# =============================================================================

create_comprehensive_team_profile_plots <- function(data, available_vars) {
  
  cat("\n=== TEAM PROFILE PLOTS FÜR ALLE TEIL-EXPERIMENTE (3 SEPARATE PLOTS) ===\n")
  
  if (length(available_vars) == 0) {
    cat("❌ Keine Variablen verfügbar\n")
    return(NULL)
  }
  
  # Bereite Basisdaten vor
  profile_data <- data %>%
    select(participant.code, experiment_simplified, all_of(available_vars)) %>%
    ...
    mutate(
      factor_clean = variable_labels[factor],
      factor_clean = ifelse(is.na(factor_clean), factor, factor_clean),
      # Kategorisierung (OHNE perceived_task_complexity_value)
      category = case_when(
        factor %in% c("interdependence_value", "common_goal_value", "means_coordination_value") ~ 
          "Flow Pre-Conditions",
        factor %in% c("group_size_value", "group_diversity_value", "group_skill_value", "team_composition_value") ~ 
          "Team Composition", 
        factor %in% c("communication_required_value", "work_independence_value", "social_presence_value") ~ 
          "Team Interaction Experience",  # KORRIGIERT: perceived_task_complexity_value entfernt
        TRUE ~ "Other"
      ),
      # Schönere Labels für Experimente + Reihenfolge festlegen
      experiment_label = case_when(
        experiment_simplified == "Exp1_Math_Team" ~ "Exp1: Math Team",
        experiment_simplified == "Exp2_Math_Team" ~ "Exp2: Math Team", 
        experiment_simplified == "Exp2_HP_Team" ~ "Exp2: HP Team",
        TRUE ~ experiment_simplified
      ),
      # Factor mit gewünschter Reihenfolge: Math Exp1, Math Exp2, HP Exp2
      experiment_label = factor(experiment_label, 
                               levels = c("Exp1: Math Team", "Exp2: Math Team", "Exp2: HP Team"))
    ) %>%
    filter(category != "Other")
  
  if (nrow(profile_data) == 0) {
    cat("❌ Keine Daten nach Filterung verfügbar\n")
    return(NULL)
  }
  
  # Farben für die drei Experimente - ANGEPASST
  colors <- c(
    "Exp1: Math Team" = "#54ACD1",      # Blau
    "Exp2: Math Team" = "#1E576F",      # Blau 
    "Exp2: HP Team" = "#E74C3C"         # Rot
  )
  
  # Berechne Mittelwerte für alle Kategorien
  mean_data <- profile_data %>%
    group_by(factor_clean, experiment_label, category) %>%
    summarise(mean_score = mean(score, na.rm = TRUE), .groups = "drop")
  
  # Funktion für einzelne Kategorie-Plots
  create_category_plot <- function(category_name) {
    
    cat("Erstelle Plot für Kategorie:", category_name, "\n")
    
    # Filtere Daten für diese Kategorie
    category_data <- profile_data %>% filter(category == category_name)
    category_means <- mean_data %>% filter(category == category_name)
    
    if (nrow(category_data) == 0) {
      cat("Keine Daten für Kategorie", category_name, "\n")
      return(NULL)
    }
    
    cat("  Variablen:", length(unique(category_data$factor_clean)), "\n")
    cat("  Beobachtungen:", nrow(category_data), "\n")
    
    # Erstelle Plot
    ggplot(category_data, aes(x = factor_clean, y = score, fill = experiment_label)) +
      geom_boxplot(position = position_dodge(width = 0.8), 
                   alpha = 0.7, 
                   outlier.shape = 16,
                   outlier.size = 1.2,
                   width = 0.75) +
      # Mittelwerte als Text
      geom_text(data = category_means, 
                aes(x = factor_clean, y = mean_score, label = round(mean_score, 1)),
                position = position_dodge(width = 0.8), 
                vjust = -0.5, 
                size = 3, 
      ...
      scale_y_continuous(limits = c(1, 7), breaks = 1:7) +
      labs(
        title = paste(category_name, "- Team Assessment Across Experiments"),
        x = "Self-Reported Construct",
        y = "Response (7Pt-Likert)",
        fill = "Experiment",
        caption = "Numbers show mean averages."
      ) +
      theme_minimal() +
      theme(
        # Bessere Lesbarkeit - horizontale Labels
        ...
        # Mehr Platz für Variablennamen
        plot.margin = margin(15, 15, 15, 15)
      )
  }
  
  # Erstelle alle drei Plots
  categories <- c("Flow Pre-Conditions", "Team Composition", "Team Interaction Experience")
  plots <- list()
  
  for (cat_name in categories) {
    plot <- create_category_plot(cat_name)
    if (!is.null(plot)) {
      plots[[cat_name]] <- plot
    }
  }
  
  cat("Erstellte Plots:", length(plots), "von", length(categories), "\n")
  
  return(plots)
}

# =============================================================================
# VERGLEICHSTABELLE - ERWEITERT FÜR DREI KATEGORIEN - KORRIGIERT
# =============================================================================

create_detailed_comparison_tables <- function(data, available_vars) {
  
  cat("\n=== DETAILLIERTE VERGLEICHSTABELLEN PRO KATEGORIE ===\n")
  
  # Berechne Descriptive Statistics
  descriptive_stats <- data %>%
    select(experiment_simplified, all_of(available_vars)) %>%
    pivot_longer(cols = all_of(available_vars), 
    ...
    mutate(
      ...
      # Kategorisierung (OHNE perceived_task_complexity_value)
      category = case_when(
        variable %in% c("interdependence_value", "common_goal_value", "means_coordination_value") ~ 
          "Flow Pre-Conditions",
        variable %in% c("group_size_value", "group_diversity_value", "group_skill_value", "team_composition_value") ~ 
          "Team Composition", 
        variable %in% c("communication_required_value", "work_independence_value", "social_presence_value") ~ 
          "Team Interaction Experience",  # KORRIGIERT
        TRUE ~ "Other"
      )
    ) %>%
    filter(category != "Other") %>%
    group_by(category, variable_clean, experiment_simplified) %>%
    summarise(
      n = n(),
  ...
  cat("Descriptive Stats Dimensionen:", nrow(descriptive_stats), "x", ncol(descriptive_stats), "\n")
  cat("Unique Kategorien:", paste(unique(descriptive_stats$category), collapse = ", "), "\n")
  cat("Unique Experimente:", paste(unique(descriptive_stats$experiment_simplified), collapse = ", "), "\n")
  
  # Erstelle separate Tabellen pro Kategorie
  categories <- unique(descriptive_stats$category)
  tables <- list()
  
  for (cat_name in categories) {
    
    cat("Erstelle Tabelle für:", cat_name, "\n")
    
    # Filtere Daten für diese Kategorie
    cat_data <- descriptive_stats %>%
      filter(category == cat_name) %>%
      select(variable_clean, experiment_simplified, mean_sd, n)
    
    cat("  Gefilterte Daten:", nrow(cat_data), "Zeilen\n")
    
    if (nrow(cat_data) > 0) {
      # Prüfe verfügbare Experimente für diese Kategorie
      available_experiments <- unique(cat_data$experiment_simplified)
      cat("  Verfügbare Experimente:", paste(available_experiments, collapse = ", "), "\n")
      
      # Erstelle Pivot-Tabelle nur mit verfügbaren Experimenten
      tryCatch({
        pivot_data <- cat_data %>%
          select(variable_clean, experiment_simplified, mean_sd) %>%
          pivot_wider(names_from = experiment_simplified, 
                      values_from = mean_sd,
                      names_prefix = "Mean_SD_")
        
        # Sample sizes separat
        n_data <- cat_data %>%
          select(variable_clean, experiment_simplified, n) %>%
          pivot_wider(names_from = experiment_simplified, 
                      values_from = n,
                      names_prefix = "N_")
        
        # Kombiniere beide
        combined_table <- pivot_data %>%
          left_join(n_data, by = "variable_clean")
        
        tables[[cat_name]] <- combined_table
        cat("  ✓ Tabelle erstellt mit", nrow(combined_table), "Variablen\n")
        
      }, error = function(e) {
        cat("  ❌ Fehler beim Erstellen der Tabelle:", e$message, "\n")
        # Fallback: Einfache Tabelle ohne Pivot
        tables[[cat_name]] <- cat_data
      })
    }
  }
...
# Erstelle formatierte Tabellen - KORRIGIERT
format_category_tables <- function(table_data) {
  
  formatted_tables <- list()
  
  for (cat_name in names(table_data$tables)) {
    
    table <- table_data$tables[[cat_name]]
    
    if (nrow(table) > 0) {
      
      # Prüfe welche Spalten verfügbar sind
      mean_cols <- names(table)[grepl("^Mean_SD_", names(table))]
      n_cols <- names(table)[grepl("^N_", names(table))]
      
      cat("Formatiere Tabelle für", cat_name, "- Mean Spalten:", length(mean_cols), "N Spalten:", length(n_cols), "\n")
      cat("  Mean Spalten:", paste(mean_cols, collapse = ", "), "\n")
      cat("  N Spalten:", paste(n_cols, collapse = ", "), "\n")
      
      if (length(mean_cols) > 0 && length(n_cols) > 0) {
        # Normale formatierte Tabelle - KORRIGIERT: Spaltenanzahl prüfen
        
        # Erstelle korrekte Spaltennamen
        mean_names <- gsub("^Mean_SD_", "", mean_cols)
        n_names <- paste0("N (", gsub("^N_", "", n_cols), ")")
        
        # Gesamtzahl der Spalten: 1 (Variable) + Mean Spalten + N Spalten
        total_cols <- 1 + length(mean_cols) + length(n_cols)
        col_names <- c("Variable", mean_names, n_names)
        
        cat("  Erwartete Spalten:", total_cols, "Erstellte col_names:", length(col_names), "\n")
        
        # Prüfe ob Anzahl stimmt
        selected_table <- table %>% select(variable_clean, all_of(mean_cols), all_of(n_cols))
        actual_cols <- ncol(selected_table)
        
        cat("  Tatsächliche Tabellenspalten:", actual_cols, "\n")
        
        if (length(col_names) == actual_cols) {
          formatted_table <- selected_table %>%
            kable(
              col.names = col_names,
              caption = paste(cat_name, ": Descriptive Statistics Across Experiments"),
              format = "html"
            ) %>%
            kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                          full_width = TRUE) %>%
            add_header_above(c(" " = 1, "Mean ± SD" = length(mean_cols), "Sample Size" = length(n_cols)))
        } else {
          cat("  ❌ Spaltenanzahl stimmt nicht überein - verwende einfache Tabelle\n")
          # Fallback: Einfache Tabelle ohne Header
          formatted_table <- selected_table %>%
            kable(
              caption = paste(cat_name, ": Descriptive Statistics Across Experiments"),
              format = "html"
        ...
        formatted_table <- table %>%
          kable(
            caption = paste(cat_name, ": Descriptive Statistics"),
            format = "html"
          ) %>%
          kable_styling(bootstrap_options = c("striped", "hover", "condensed"), 
                        full_width = TRUE)
      ...
      formatted_tables[[cat_name]] <- formatted_table
    }
  }
...
# =============================================================================
# DURCHFÜHRUNG DER ERWEITERTEN ANALYSE - KORRIGIERT
# =============================================================================

# Hauptanalyse
if (exists("integrated_data_full")) {
  
  # Bereite Daten für alle Experimente vor
  all_experiments_result <- prepare_all_experiments_data(integrated_data_full)
  
  if (!is.null(all_experiments_result)) {
    
    # Erstelle drei separate Team Profile Plots
    team_profile_plots <- create_comprehensive_team_profile_plots(
      all_experiments_result$data, 
      all_experiments_result$available_vars
    )
    
    if (!is.null(team_profile_plots) && length(team_profile_plots) > 0) {
      
      # Zeige alle Plots
      for (plot_name in names(team_profile_plots)) {
        cat("\n--- Plot für", plot_name, "---\n")
        print(team_profile_plots[[plot_name]])
        
        # Speichere jeden Plot separat
        filename <- paste0("team_profile_", gsub(" ", "_", tolower(plot_name)), ".png")
        ggsave(filename, team_profile_plots[[plot_name]], 
               width = 12, height = 6, dpi = 300, bg = "white")
        cat("✓ Plot gespeichert als", filename, "\n")
      }
      
    } else {
      cat("❌ Keine Team Profile Plots erstellt\n")
    }
    
    # Erstelle detaillierte Vergleichstabellen pro Kategorie
    detailed_tables <- create_detailed_comparison_tables(
      all_experiments_result$data,
      all_experiments_result$available_vars
    )
    
    # Formatiere und zeige Tabellen
    if (!is.null(detailed_tables) && length(detailed_tables$tables) > 0) {
      
      formatted_tables <- format_category_tables(detailed_tables)
      
      if (length(formatted_tables) > 0) {
        for (table_name in names(formatted_tables)) {
          cat("\n=== TABELLE:", table_name, "===\n")
          print(formatted_tables[[table_name]])
        }
        
        # Exportiere detaillierte Daten
        write.csv(detailed_tables$descriptive_stats, "team_factors_detailed_stats.csv", row.names = FALSE)
        
        # Exportiere separate CSV pro Kategorie
        for (cat_name in names(detailed_tables$tables)) {
          filename <- paste0("team_factors_", gsub(" ", "_", tolower(cat_name)), ".csv")
          write.csv(detailed_tables$tables[[cat_name]], filename, row.names = FALSE)
          cat("✓ Tabelle exportiert als", filename, "\n")
        }
        
      } else {
        cat("❌ Keine formatierten Tabellen erstellt\n")
      }
    } else {
      cat("❌ Keine Tabellen erstellt\n")
    }
    
  } else {
    cat("❌ Konnte Daten für alle Experimente nicht vorbereiten\n")
  }
  
} else {
  cat("❌ integrated_data_full nicht gefunden\n")
  cat("Bitte lade zuerst deine integrierten Daten\n")
}

# =============================================================================
# 2. TEAM PROFILE PLOTS EXPORTIEREN  
# =============================================================================

cat("\n--- EXPORTIERE TEAM PROFILE PLOTS ---\n")

# Team Profile Plots (falls vorhanden)
if (exists("team_profile_plots") && !is.null(team_profile_plots)) {
  
  # Team Profile Plots sind breiter, daher andere Dimensionen
  team_width_paper <- 10
  team_height_paper <- 6
  team_width_presentation <- 14
  team_height_presentation <- 8
  
  for (plot_name in names(team_profile_plots)) {
    plot_obj <- team_profile_plots[[plot_name]]
    
    if (!is.null(plot_obj)) {
      # Sauberer Dateiname (entferne Sonderzeichen)
      clean_name <- gsub("[^A-Za-z0-9_]", "_", plot_name)
      clean_name <- gsub("_{2,}", "_", clean_name)  # Mehrfache Unterstriche entfernen
      clean_name <- tolower(clean_name)
      
      # Für Publikationen (TIFF)
      tryCatch({
        ggsave(file.path("plots_export", paste0("team_", clean_name, "_publication.tiff")), 
               plot = plot_obj,
               width = team_width_paper, height = team_height_paper, 
               dpi = paper_dpi, compression = "lzw", bg = "white")
        cat("✓ Exportiert: team_", clean_name, "_publication.tiff\n")
      }, error = function(e) {
        cat("❌ Fehler beim Export von", plot_name, "TIFF:", e$message, "\n")
      })
      
      # Für Präsentationen (PNG)
      tryCatch({
        ggsave(file.path("plots_export", paste0("team_", clean_name, "_presentation.png")), 
               plot = plot_obj,
               width = team_width_presentation, height = team_height_presentation, 
               dpi = presentation_dpi, bg = "white")
        cat("✓ Exportiert: team_", clean_name, "_presentation.png\n")
      }, error = function(e) {
        cat("❌ Fehler beim Export von", plot_name, "PNG:", e$message, "\n")
      })
      ...
      # Für Vector Graphics (PDF)
      tryCatch({
        ggsave(file.path("plots_export", paste0("team_", clean_name, "_vector.pdf")), 
               plot = plot_obj,
               width = team_width_paper, height = team_height_paper, 
               bg = "white")
        cat("✓ Exportiert: team_", clean_name, "_vector.pdf\n")
      }, error = function(e) {
        cat("❌ Fehler beim Export von", plot_name, "PDF:", e$message, "\n")
      })
    }
  ...
  cat("⚠️ Keine Team Profile Plots gefunden\n")
}

# =============================================================================
# A1.2 ZUSAMMENFASSUNG
# =============================================================================

cat("\n=== A1.2 TEAM INTERACTION FORMAT ASSESSMENT ZUSAMMENFASSUNG ===\n")

# Zähle erfolgreiche Analysen
cat("Abgeschlossene Analysen:\n")
if (exists("team_profile_plots")) cat("✓ Team Profile Plots erstellt:", length(team_profile_plots), "\n")
if (exists("detailed_tables")) cat("✓ Detaillierte Tabellen erstellt:", length(detailed_tables$tables), "\n")

cat("\nAnalysierte Experimente:\n")
cat("✓ Experiment 1: Math Team (Together_None)\n")
cat("✓ Experiment 2: Math Team (Together_Chat + Together_Jitsi)\n") 
cat("✓ Experiment 2: HP Team (Together_Chat + Together_Jitsi)\n")

cat("\n💡 Team Interaction Format Assessment Interpretation:\n")
cat("• Vergleich zwischen Experimenten und Kommunikationsformaten\n")
cat("• Flow Pre-Conditions, Team Composition, und Interaction Experience analysiert\n")
cat("• Basis für Verständnis der Kommunikationseffekte auf Teamdynamik\n")

cat("\n=== A1.2 TEAM INTERACTION FORMAT ASSESSMENT ABGESCHLOSSEN ===\n")
```
