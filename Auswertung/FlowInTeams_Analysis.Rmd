...
alpha_work <- psych::alpha(flowp_items %>% dplyr::select(starts_with("Outro.1.player.fpw")))
alpha_household <- psych::alpha(flowp_items %>% dplyr::select(starts_with("Outro.1.player.fph")))
alpha_leisure <- psych::alpha(flowp_items %>% dplyr::select(starts_with("Outro.1.player.fpl")))
...
compare_bs_effects <- function(original, clean, task_name) {
  ...
  comparison <- original %>%
    dplyr::select(difficulty, estimate, p.value, p.adj) %>%
...
if (nrow(all_significant_bs_clean) > 0) {
  ...
  print(all_significant_bs_clean %>% dplyr::select(Task, difficulty, estimate, p.value, p.adj))
} else {
...
# Gender composition zu flow_scores hinzufügen (sollte bereits existieren)
if(!"gender_comp" %in% names(flow_scores)) {
